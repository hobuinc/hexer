...
#define BOOST_TYPEOF_modifier_support(ID, Fun)\
    ...
    {\
        typedef\
            typename hexerboost::type_of::encode_type<\
            typename hexerboost::type_of::push_back<\
            V\
            , hexerboost::mpl::size_t<ID> >::type\
            , T>::type\
            type;\
    };\
    template<class Iter> struct decode_type_impl<hexerboost::mpl::size_t<ID>, Iter>\
    {\
        typedef hexerboost::type_of::decode_type<Iter> d1;\
...
#define BOOST_TYPEOF_array_support(ID, Qualifier)\
    ...
    {\
        typedef\
            ...
            V\
            , hexerboost::mpl::size_t<ID> >::type\
            , hexerboost::mpl::size_t<N> >::type\
            , T>::type\
        type;\
    };\
    template<class Iter>\
    struct decode_type_impl<hexerboost::mpl::size_t<ID>, Iter>\
    {\
        enum{n = Iter::type::value};\
        typedef hexerboost::type_of::decode_type<typename Iter::next> d;\
