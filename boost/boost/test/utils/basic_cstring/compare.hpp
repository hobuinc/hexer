...
template<class CharT>
inline bool
operator <( hexerboost::unit_test::basic_cstring<CharT> const& x,
            hexerboost::unit_test::basic_cstring<CharT> const& y )
{
    typedef typename hexerboost::unit_test::basic_cstring<CharT>::traits_type traits_type;
    return x.size() != y.size() 
            ? x.size() < y.size() 
