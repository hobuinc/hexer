...
public:
    ...
    void            filter_unit( test_unit const& tu )
    {
        ...
        tu.p_enabled.value =
            std::find_if( filters.begin(), filters.end(), bind( &single_filter::pass, _1, hexerboost::ref(tu) ) ) != filters.end();
    }
...
unit_test_main( init_unit_test_func init_func, int argc, char* argv[] )
{
    try {
        ...
        return runtime_config::no_result_code() 
                    ? hexerboost::exit_success
                    : results_collector.results( framework::master_test_suite().p_id ).result_code();
    }
    catch( framework::nothing_to_test const& ) {
        return hexerboost::exit_success;
    }
...
main( int argc, char* argv[] )
{
    ...
    hexerboost::unit_test::init_unit_test_func init_func = &init_unit_test;
#else
    extern ::hexerboost::unit_test::test_suite* init_unit_test_suite( int argc, char* argv[] );

    hexerboost::unit_test::init_unit_test_func init_func = &init_unit_test_suite;
#endif

    return ::hexerboost::unit_test::unit_test_main( init_func, argc, argv );
}
