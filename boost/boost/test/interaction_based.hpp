...
#define BOOST_ITEST_EPOINT( description ) \
    ::hexerboost::itest::manager::instance().exception_point( BOOST_TEST_L(__FILE__), __LINE__, description )
/**/
...
#define BOOST_ITEST_DPOINT() \
    ::hexerboost::itest::manager::instance().decision_point( BOOST_TEST_L(__FILE__), __LINE__ )
/**/
...
#define BOOST_ITEST_SCOPE( scope_name ) \
    ::hexerboost::itest::scope_guard itest_scope_guard ## __LINE__( BOOST_TEST_L(__FILE__), __LINE__, BOOST_STRINGIZE(scope_name) )
/**/
...
#define BOOST_ITEST_NEW( type_name ) \
    new ( ::hexerboost::itest::location( BOOST_TEST_L(__FILE__), __LINE__ ) ) type_name
/**/
...
#define BOOST_ITEST_DATA_FLOW( v ) \
    ::hexerboost::itest::manager::instance().generic_data_flow( v )
/**/
...
#define BOOST_ITEST_RETURN( type, default_value ) \
    ::hexerboost::itest::manager::instance().generic_return<type>( default_value )
/**/
...
#define BOOST_ITEST_MOCK_FUNC( function_name )          \
    ...
    return ::hexerboost::itest::mock_object<>::prototype();  \
/**/
...
inline void*
operator new( std::size_t s, ::hexerboost::itest::location const& l )
{
    void* res = std::malloc(s ? s : 1);

    if( res )
        ::hexerboost::itest::manager::instance().allocated( l.m_file_name, l.m_line_num, res, s );
    else
        throw std::bad_alloc();
...
inline void
operator delete( void* p, ::hexerboost::itest::location const& )
{
    ::hexerboost::itest::manager::instance().freed( p );
