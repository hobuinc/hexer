...
public:
    // Constructor
    explicit    translate_exception_base( hexerboost::scoped_ptr<translate_exception_base>& next )
    {
        next.swap( m_next );
    }
...
protected:
    // Data members
    hexerboost::scoped_ptr<translate_exception_base> m_next;
};
...
class BOOST_TEST_DECL execution_exception {
    typedef hexerboost::unit_test::const_string const_string;
public:
    enum error_code {
...
public:
    ...
    void        register_exception_translator( ExceptionTranslator const& tr, hexerboost::type<Exception>* = 0 );
...
private:
    ...
    // Data members
    hexerboost::scoped_ptr<detail::translate_exception_base> m_custom_translators;
    hexerboost::scoped_array<char>                           m_alt_stack;
}; // execution_monitor
...
{
    typedef hexerboost::scoped_ptr<translate_exception_base> base_ptr;
public:
    ...
    int operator()( unit_test::callback0<int> const& F )
    {
        try {
            ...
            m_translator( e );
            return hexerboost::exit_exception_failure;
        }
    }
...
template<typename Exception, typename ExceptionTranslator>
void
execution_monitor::register_exception_translator( ExceptionTranslator const& tr, hexerboost::type<Exception>* )
{
...
#define BOOST_TEST_SYS_ASSERT( exp ) if( (exp) ) ; else throw ::hexerboost::system_error( BOOST_STRINGIZE( exp ) )
