...
#include <boost/type_traits/is_floating_point.hpp> // for hexerboost::is_floating_point;
#include <boost/type_traits/is_integral.hpp> // for hexerboost::is_integral
#include <boost/type_traits/is_convertible.hpp> // for hexerboost::is_convertible
#include <boost/type_traits/is_same.hpp>// for hexerboost::is_same
#include <boost/type_traits/remove_cv.hpp>// for hexerboost::remove_cv
// Boost Template meta programming:
#include <boost/mpl/if.hpp> // for hexerboost::mpl::if_c.
#include <boost/mpl/and.hpp> // for hexerboost::mpl::if_c.
#include <boost/mpl/or.hpp> // for hexerboost::mpl::if_c.
#include <boost/mpl/not.hpp> // for hexerboost::mpl::if_c.
    ...
    namespace tools
    {
      ...
      { // Promote, if necessary, & pick the wider of the two floating-point types.
        ...
        typedef typename mpl::if_<
          ...
          typename mpl::if_< typename mpl::and_<mpl::not_<is_floating_point<T2P> >, ::hexerboost::is_convertible<T1P, T2P> >, T2P, T1P>::type>::type type;
      }; // promote_arg2
      // These full specialisations reduce mpl::if_ usage and speed up
      // compilation:
      ...
      struct promote_args
      {
         ...
         //
         BOOST_STATIC_ASSERT((0 == ::hexerboost::is_same<type, long double>::value));
#endif
      };
