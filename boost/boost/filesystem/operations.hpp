...
# endif
  ...
  struct space_info
  {
    ...
    hexerboost::uintmax_t capacity;
    hexerboost::uintmax_t free;      // <= capacity
    hexerboost::uintmax_t available; // <= free
  };
...
//--------------------------------------------------------------------------------------//
  ...
  namespace detail
  {
    ...
    BOOST_FILESYSTEM_DECL
    hexerboost::uintmax_t file_size(const path& p, system::error_code* ec=0);
    BOOST_FILESYSTEM_DECL
    hexerboost::uintmax_t hard_link_count(const path& p, system::error_code* ec=0);
    BOOST_FILESYSTEM_DECL
    ...
    BOOST_FILESYSTEM_DECL
    hexerboost::uintmax_t remove_all(const path& p, system::error_code* ec=0);
    BOOST_FILESYSTEM_DECL
...
# endif
  ...
  inline
  hexerboost::uintmax_t file_size(const path& p) {return detail::file_size(p);}

  inline
  hexerboost::uintmax_t file_size(const path& p, system::error_code& ec)
                                       {return detail::file_size(p, &ec);}
  inline
  hexerboost::uintmax_t hard_link_count(const path& p) {return detail::hard_link_count(p);}
  ...
  inline
  hexerboost::uintmax_t remove_all(const path& p) {return detail::remove_all(p);}
    
  inline
  hexerboost::uintmax_t remove_all(const path& p, system::error_code& ec)
...
public:
  ...
  directory_entry() {}
  explicit directory_entry(const hexerboost::filesystem::path& p,
    file_status st = file_status(), file_status symlink_st=file_status())
    : m_path(p), m_status(st), m_symlink_status(symlink_st)
    {}

  void assign(const hexerboost::filesystem::path& p,
    file_status st = file_status(), file_status symlink_st = file_status())
    { m_path = p; m_status = st; m_symlink_status = symlink_st; }

  void replace_filename(const hexerboost::filesystem::path& p,
    file_status st = file_status(), file_status symlink_st = file_status())
  {
...
# ifndef BOOST_FILESYSTEM_NO_DEPRECATED
  void replace_leaf(const hexerboost::filesystem::path& p,
    file_status st, file_status symlink_st)
      { replace_filename(p, st, symlink_st); }
# endif

  const hexerboost::filesystem::path&  path() const               {return m_path;}
...
private:
  hexerboost::filesystem::path   m_path;
...
//--------------------------------------------------------------------------------------//
  ...
  class directory_iterator
    : public hexerboost::iterator_facade< directory_iterator,
                                     directory_entry,
  ...
  private:
    ...
    hexerboost::shared_ptr< detail::dir_itr_imp >  m_imp;

    friend class hexerboost::iterator_core_access;

    hexerboost::iterator_facade<
      directory_iterator,
      directory_entry,
      hexerboost::single_pass_traversal_tag >::reference dereference() const 
    {
...
//--------------------------------------------------------------------------------------//
  ...
  namespace detail
  {
    ...
    //  clients of struct 'hexerboost::filesystem::detail::recur_dir_itr_imp'
...
//--------------------------------------------------------------------------------------//
  ...
  class recursive_directory_iterator
    : public hexerboost::iterator_facade<
        recursive_directory_iterator,
        directory_entry,
        hexerboost::single_pass_traversal_tag >
  {
...
#   endif
  ...
  private:
    ...
    // shared_ptr provides shallow-copy semantics required for InputIterators.
    // m_imp.get()==0 indicates the end iterator.
    hexerboost::shared_ptr< detail::recur_dir_itr_imp >  m_imp;
    ...
    hexerboost::iterator_facade< 
      recursive_directory_iterator,
      directory_entry,
      hexerboost::single_pass_traversal_tag >::reference
    dereference() const 
    {
      BOOST_ASSERT_MSG(m_imp.get(),
...
//--------------------------------------------------------------------------------------//
  ...
  private:
    ...
    hexerboost::shared_ptr<m_imp> m_imp_ptr;
  };
