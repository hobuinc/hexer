...
class condition_any_algorithm
{
   ...
   template <class Lock>
   static bool do_timed_wait(ConditionAnyMembers &data, Lock& lock, const hexerboost::posix_time::ptime &abs_time);

   public:
   template<class Lock>
   static bool wait  ( ConditionAnyMembers &data, Lock &mut
                     , bool timeout_enabled, const hexerboost::posix_time::ptime &abs_time);
...
bool condition_any_algorithm<ConditionAnyMembers>::do_timed_wait
   (ConditionAnyMembers &data, Lock& lock, const hexerboost::posix_time::ptime &abs_time)
{
   //lock internal before unlocking external to avoid race with a notifier
   scoped_lock<mutex_type> internal_lock(data.get_mutex());
   {
...
class condition_any_wrapper
{
   ...
   void wait(L& lock)
   {
      ...
      algo_type::wait(m_data, lock, false, hexerboost::posix_time::ptime());
   }
