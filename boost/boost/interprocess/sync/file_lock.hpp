...
class file_lock
{
   ...
   {
      file_lock tmp(hexerboost::move(moved));
      this->swap(tmp);
      return *this;
   }
   ...
   //!Throws: interprocess_exception on error.
   bool timed_lock_sharable(const hexerboost::posix_time::ptime &abs_time);
   ...
   bool timed_acquire_file_lock
      (file_handle_t hnd, bool &acquired, const hexerboost::posix_time::ptime &abs_time)
   {
      //Obtain current count and target time
      hexerboost::posix_time::ptime now = microsec_clock::universal_time();
      using namespace hexerboost::detail;
...
inline bool file_lock::timed_lock(const hexerboost::posix_time::ptime &abs_time)
{
...
inline bool file_lock::timed_lock_sharable(const hexerboost::posix_time::ptime &abs_time)
{
   if(abs_time == hexerboost::posix_time::pos_infin){
      this->lock_sharable();
      return true;
