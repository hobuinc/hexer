...
inline void spin_recursive_mutex::lock()
{
   ...
   if(ipcdetail::equal_systemwide_thread_id(thr_id , old_id)){
      if((unsigned int)(m_nLockCount+1) == 0){
         ...
         throw interprocess_exception("hexerboost::interprocess::spin_recursive_mutex recursive lock overflow");
      }
      ++m_nLockCount;
   }
   else{
...
inline bool spin_recursive_mutex::timed_lock(const hexerboost::posix_time::ptime &abs_time)
{
   typedef ipcdetail::OS_systemwide_thread_id_t handle_t;
