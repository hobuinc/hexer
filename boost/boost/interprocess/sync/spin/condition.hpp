...
class spin_condition
{
   ...
   template <typename L>
   bool timed_wait(L& lock, const hexerboost::posix_time::ptime &abs_time)
   {
   ...
   bool timed_wait(L& lock, const hexerboost::posix_time::ptime &abs_time, Pr pred)
   {
      if(abs_time == hexerboost::posix_time::pos_infin){
         this->wait(lock, pred);
         return true;
   ...
   template<class InterprocessMutex>
   bool do_timed_wait(const hexerboost::posix_time::ptime &abs_time, InterprocessMutex &mut);

   private:
   template<class InterprocessMutex>
   bool do_timed_wait(bool tout_enabled, const hexerboost::posix_time::ptime &abs_time, InterprocessMutex &mut);

   enum { SLEEP = 0, NOTIFY_ONE, NOTIFY_ALL };
   spin_mutex  m_enter_mut;
   volatile hexerboost::uint32_t    m_command;
   volatile hexerboost::uint32_t    m_num_waiters;
   void notify(hexerboost::uint32_t command);
};
...
inline void spin_condition::notify(hexerboost::uint32_t command)
{
   ...
   while(SLEEP != atomic_cas32(const_cast<hexerboost::uint32_t*>(&m_command), command, SLEEP)){
      thread_yield();
   }
/*
   //Wait until the threads are woken
   while(SLEEP != atomic_cas32(const_cast<hexerboost::uint32_t*>(&m_command), 0)){
      thread_yield();
   }
*/
...
{
   this->do_timed_wait(false, hexerboost::posix_time::ptime(), mut);
}

template<class InterprocessMutex>
inline bool spin_condition::do_timed_wait
   (const hexerboost::posix_time::ptime &abs_time, InterprocessMutex &mut)
{
...
inline bool spin_condition::do_timed_wait(bool tout_enabled,
                                     const hexerboost::posix_time::ptime &abs_time,
                                     InterprocessMutex &mut)
{
   ...
   typedef hexerboost::interprocess::scoped_lock<spin_mutex> InternalLock;
   ...
   //no other thread can execute the do_timed_wait method.
   {
      ...
      atomic_inc32(const_cast<hexerboost::uint32_t*>(&m_num_waiters));
   ...
   while(1){
      ...
      if(tout_enabled && timed_out){
         ...
         atomic_dec32(const_cast<hexerboost::uint32_t*>(&m_num_waiters));
         unlock_enter_mut = true;
         break;
      }
      else{
         hexerboost::uint32_t result = atomic_cas32
                        (const_cast<hexerboost::uint32_t*>(&m_command), SLEEP, NOTIFY_ONE);
         if(result == SLEEP){
            ...
            unlock_enter_mut = 1 == atomic_dec32(const_cast<hexerboost::uint32_t*>(&m_num_waiters));
            //Check if this is the last thread of notify_all waiters
            //Only the last thread will release the mutex
            if(unlock_enter_mut){
               atomic_cas32(const_cast<hexerboost::uint32_t*>(&m_command), SLEEP, NOTIFY_ALL);
            }
            break;
         }
      }
