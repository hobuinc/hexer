...
class spin_semaphore
{
   ...
   void wait();
   bool try_wait();
   bool timed_wait(const hexerboost::posix_time::ptime &abs_time);

//   int get_count() const;
   private:
   volatile hexerboost::uint32_t m_count;
};
...
{  ipcdetail::atomic_write32(&this->m_count, hexerboost::uint32_t(initialCount));  }
...
{
   while(!ipcdetail::atomic_add_unless32(&m_count, hexerboost::uint32_t(-1), hexerboost::uint32_t(0))){
...
{
   return ipcdetail::atomic_add_unless32(&m_count, hexerboost::uint32_t(-1), hexerboost::uint32_t(0));
}

inline bool spin_semaphore::timed_wait(const hexerboost::posix_time::ptime &abs_time)
{
