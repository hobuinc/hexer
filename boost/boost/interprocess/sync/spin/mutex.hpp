...
class spin_mutex
{
   ...
   void take_ownership(){};
   private:
   volatile hexerboost::uint32_t m_s;
};
...
{
   do{
      hexerboost::uint32_t prev_s = ipcdetail::atomic_cas32(const_cast<hexerboost::uint32_t*>(&m_s), 1, 0);
...
inline bool spin_mutex::timed_lock(const hexerboost::posix_time::ptime &abs_time)
{
...
{  ipcdetail::atomic_cas32(const_cast<hexerboost::uint32_t*>(&m_s), 0, 1);   }
