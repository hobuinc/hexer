...
class xsi_named_mutex
{
   ...
   xsi_named_mutex(open_or_create_t, const char *path, hexerboost::uint8_t id, int perm = 0666)
   ...
   {
      xsi_named_mutex tmp(hexerboost::move(moved));
      this->swap(tmp);
      return *this;
   }
...
inline bool xsi_named_mutex::priv_open_or_create
   (ipcdetail::create_enum_t type, const char *path, hexerboost::uint8_t id, int perm)
{
   key_t key;
   if(path){
      key  = ::ftok(path, id);
