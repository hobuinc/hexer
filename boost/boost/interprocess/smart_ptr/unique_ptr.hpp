...
class unique_ptr
{
   ...
   unique_ptr(BOOST_RV_REF(unique_ptr) u)
      : ptr_(u.release(), hexerboost::forward<D>(u.get_deleter()))
   {}
   ...
   unique_ptr(BOOST_RV_REF_2_TEMPL_ARGS(unique_ptr, U, E) u,
      typename ipcdetail::enable_if_c<
            ...
            nat
            >::type = nat())
      : ptr_(const_cast<unique_ptr<U,E>&>(u).release(), hexerboost::move<D>(u.get_deleter()))
   {}
   ...
   {
      reset(u.release());
      ptr_.second() = hexerboost::move(u.get_deleter());
      return *this;
   }
   ...
   /// @cond
   private:
   hexerboost::compressed_pair<pointer, D> ptr_;
...
private:
    ...
    template <class U, class E> unique_ptr(U p, E,
        typename hexerboost::enable_if<hexerboost::is_convertible<U, pointer> >::type* = 0);
...
//!of type T with hexerboost::interprocess::deleter deleter
...
//!with hexerboost::interproces::deleter from a pointer
