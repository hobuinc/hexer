...
class shared_count
{
   ...
   typedef hexerboost::container::allocator_traits<VoidAllocator>         vallocator_traits;
...
/*
      ...
      : m_pi( r.m_pi )
   {
      if( m_pi == 0 || !m_pi->add_ref_lock() ){
         hexerboost::throw_exception( hexerboost::interprocess::bad_weak_ptr() );
      }
   }
*/
   template<class Y>
   ...
   {
      if( m_pi == 0 || !m_pi->add_ref_lock() ){
         throw( hexerboost::interprocess::bad_weak_ptr() );
      }
   }
