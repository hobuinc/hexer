...
private:
    ...
    hexerboost::uint32_t use_count_;        // #shared
    hexerboost::uint32_t weak_count_;       // #weak + (#shared != 0)
...
public:
    ...
    bool add_ref_lock() // true on success
    {
        for( ;; )
        {
            hexerboost::uint32_t tmp = static_cast< hexerboost::uint32_t const volatile& >( use_count_ );
   ...
   { return (long)static_cast<hexerboost::uint32_t const volatile &>( use_count_ ); }
};
