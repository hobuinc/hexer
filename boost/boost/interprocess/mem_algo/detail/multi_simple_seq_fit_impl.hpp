...
class simple_seq_fit_impl
{
   ...
   static const std::size_t Alignment = ::hexerboost::alignment_of<hexerboost::ipcdetail::max_align>::value;
...
inline std::pair<void *, bool> simple_seq_fit_impl<MutexFamily, VoidPointer>::
   allocation_command  (hexerboost::interprocess::allocation_type command,   size_type min_size,
                        size_type preferred_size,size_type &received_size, 
   ...
   multi_allocate(size_type nbytes)
{
   ...
   //since it's faster.
   size_type ignore;
   void *addr = this->priv_allocate(hexerboost::interprocess::allocate_new, nbytes, nbytes, ignore).first;
   if(!addr){
      ...
      hexerboost::interprocess::multi_segment_services *p_services = 
         static_cast<hexerboost::interprocess::multi_segment_services*>
     ...
     std::pair<void *, std::size_t> ret = 
      ...
      if(ret.first){
         ...
         addr = this->priv_allocate(hexerboost::interprocess::allocate_new, nbytes, nbytes, ignore).first;
      }
   }
   return addr;
}
...
{  
   ...
   void *buffer = priv_allocate(hexerboost::interprocess::allocate_new, request, request, ignore).first;
   if(!buffer)
      return 0;
