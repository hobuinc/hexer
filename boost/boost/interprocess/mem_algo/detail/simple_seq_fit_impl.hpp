...
class simple_seq_fit_impl
{
   ...
   typedef typename hexerboost::intrusive::
   ...
   typedef hexerboost::container::container_detail::
      basic_multiallocation_chain<VoidPointer>     multiallocation_chain;

   typedef typename hexerboost::intrusive::pointer_traits<char_ptr>::difference_type difference_type;
   typedef typename hexerboost::make_unsigned<difference_type>::type size_type;
   ...
   {
      //-----------------------
      hexerboost::interprocess::scoped_lock<interprocess_mutex> guard(m_header);
      //-----------------------
   ...
   template<class T>
   std::pair<T *, bool>
      allocation_command  (hexerboost::interprocess::allocation_type command,   size_type limit_size,
                           size_type preferred_size,size_type &received_size,
                           T *reuse_ptr = 0);

   std::pair<void *, bool>
      raw_allocation_command  (hexerboost::interprocess::allocation_type command,   size_type limit_size,
                               size_type preferred_size,size_type &received_size,
   ...
   std::pair<void *, bool> priv_allocate(hexerboost::interprocess::allocation_type command
                                        ,size_type min_size
                                        ,size_type preferred_size
                                        ,size_type &received_size
                                        ,void *reuse_ptr = 0);

   std::pair<void *, bool> priv_allocation_command(hexerboost::interprocess::allocation_type command
                                        ,size_type min_size
                                        ,size_type preferred_size
                                        ,size_type &received_size
                                        ,void *reuse_ptr
   ...
   void* priv_expand_both_sides(hexerboost::interprocess::allocation_type command
                               ,size_type min_size
                               ,size_type preferred_size
                               ,size_type &received_size
                               ,void *reuse_ptr
   ...
   public:
	static const size_type Alignment      = ::hexerboost::alignment_of< ::hexerboost::detail::max_align>::value;
   private:
...
void simple_seq_fit_impl<MutexFamily, VoidPointer>::shrink_to_fit()
{
   ...
   if(!m_header.m_allocated){
      ...
      size_type ignore;
      unique_block = priv_allocate(hexerboost::interprocess::allocate_new, 0, 0, ignore).first;
      if(!unique_block)
         return;
   ...
   allocate(size_type nbytes)
{
   ...
   //-----------------------
   size_type ignore;
   return priv_allocate(hexerboost::interprocess::allocate_new, nbytes, nbytes, ignore).first;
}
                        ...
                        T *reuse_ptr)
{
   ...
   BOOST_ASSERT(0 == ((std::size_t)ret.first % ::hexerboost::alignment_of<T>::value));
...
inline std::pair<void*, bool> simple_seq_fit_impl<MutexFamily, VoidPointer>::
   raw_allocation_command  (hexerboost::interprocess::allocation_type command,   size_type limit_objects,
                        ...
                        void *reuse_ptr, size_type sizeof_object)
{
   ...
   if(command & hexerboost::interprocess::try_shrink_in_place){
...
template<class MutexFamily, class VoidPointer>
inline std::pair<void*, bool> simple_seq_fit_impl<MutexFamily, VoidPointer>::
   priv_allocation_command (hexerboost::interprocess::allocation_type command,   size_type limit_size,
                       size_type preferred_size, size_type &received_size,
                       void *reuse_ptr, size_type sizeof_object)
{
   command &= ~hexerboost::interprocess::expand_bwd;
...
template<class MutexFamily, class VoidPointer>
void* simple_seq_fit_impl<MutexFamily, VoidPointer>::
   priv_expand_both_sides(hexerboost::interprocess::allocation_type command
                         ,size_type min_size
                         ,size_type preferred_size
                         ,size_type &received_size
                         ,void *reuse_ptr
                         ,bool only_preferred_backwards)
{
   ...
   if(command & hexerboost::interprocess::expand_fwd){
   ...
   if(command & hexerboost::interprocess::expand_bwd){
...
std::pair<void *, bool> simple_seq_fit_impl<MutexFamily, VoidPointer>::
   priv_allocate(hexerboost::interprocess::allocation_type command
                ,size_type limit_size
                ,size_type preferred_size
                ,size_type &received_size
                ,void *reuse_ptr)
{
   if(command & hexerboost::interprocess::shrink_in_place){
      bool success =
   ...
   //
   if(reuse_ptr && (command & (hexerboost::interprocess::expand_fwd | hexerboost::interprocess::expand_bwd))){
   ...
   if(command & hexerboost::interprocess::allocate_new){
      received_size = 0;
      while(block != root){
