...
class basic_multiallocation_chain
   : public hexerboost::container::container_detail::
      basic_multiallocation_chain<VoidPointer>
{
   ...
   basic_multiallocation_chain(BOOST_RV_REF(basic_multiallocation_chain) other)
      :  base_t(::hexerboost::move(static_cast<base_t&>(other)))
   {}

   basic_multiallocation_chain& operator=(BOOST_RV_REF(basic_multiallocation_chain) other)
   {
      this->base_t::operator=(::hexerboost::move(static_cast<base_t&>(other)));
      return *this;
   }

   void *pop_front()
   {
      return hexerboost::interprocess::ipcdetail::to_raw_pointer(this->base_t::pop_front());
   }
};
      ...
      (MemoryAlgorithm *memory_algo, size_type nbytes, size_type alignment)
   {
      ...
      if(alignment <= Alignment){
         return memory_algo->priv_allocate
            (hexerboost::interprocess::allocate_new, nbytes, nbytes, real_size).first;
      }
      ...
      void *buffer = memory_algo->priv_allocate
         (hexerboost::interprocess::allocate_new, request, request, real_size).first;
      if(!buffer){
         return 0;
      }
      ...
      if(total_request_units && !multiplication_overflows(total_request_units, Alignment)){
         ...
         while(low_idx < n_elements){
            ...
            std::pair<void *, bool> ret = memory_algo->priv_allocate
               (hexerboost::interprocess::allocate_new, min_allocation, total_bytes, received_size, 0);
            if(!ret.first){
               break;
            }
