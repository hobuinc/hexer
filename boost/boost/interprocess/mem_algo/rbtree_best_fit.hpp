...
class rbtree_best_fit
{
   ...
   {
      //-----------------------
      hexerboost::interprocess::scoped_lock<mutex_type> guard(m_header);
      //-----------------------
   ...
   std::pair<void *, bool>
     raw_allocation_command  (hexerboost::interprocess::allocation_type command,   size_type limit_object,
   ...
   static const size_type Alignment = !MemAlignment
      ? size_type(::hexerboost::alignment_of< ::hexerboost::detail::max_align>::value)
   ...
   BOOST_STATIC_ASSERT((Alignment >= ::hexerboost::alignment_of<void_pointer>::value));
...
void rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::shrink_to_fit()
{
   ...
   if(priv_next_block(first_block) == old_end_block){
      ...
      size_type ignore;
      unique_buffer = priv_allocate(hexerboost::interprocess::allocate_new, 0, 0, ignore).first;
   ...
   allocate(size_type nbytes)
{
   ...
   //-----------------------
   size_type ignore;
   void * ret = priv_allocate(hexerboost::interprocess::allocate_new, nbytes, nbytes, ignore).first;
   return ret;
}
