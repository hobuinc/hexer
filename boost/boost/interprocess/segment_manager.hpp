   ...
   :  private MemoryAlgorithm
{
   ...
   //!Allocates nbytes bytes. Throws hexerboost::interprocess::bad_alloc
   //!on failure
                           ...
                           T *reuse_ptr = 0)
   {
      std::pair<T *, bool> ret = MemoryAlgorithm::allocation_command
         ( command | hexerboost::interprocess::nothrow_allocation, limit_size, preferred_size, received_size
         , reuse_ptr);
      if(!(command & hexerboost::interprocess::nothrow_allocation) && !ret.first)
         throw bad_alloc();
      return ret;
   }
                           ...
                           void *reuse_ptr = 0, size_type sizeof_object = 1)
   {
      std::pair<void *, bool> ret = MemoryAlgorithm::raw_allocation_command
         ( command | hexerboost::interprocess::nothrow_allocation, limit_objects, preferred_objects, received_objects
   ...
   :  public segment_manager_base<MemoryAlgorithm>
{
   ...
   //!Can throw hexerboost::interprocess::bad_alloc if there is no enough memory.
   ...
   struct allocator
   {
      typedef hexerboost::interprocess::allocator<T, segment_manager> type;
   };
   ...
   struct deleter
   {
      typedef hexerboost::interprocess::deleter<T, segment_manager> type;
   };
   ...
   {
      hexerboost::interprocess::allocation_type type = ctrl_data->alloc_type();
      if(type != named_type){
   ...
   scoped_lock<rmutex> priv_get_lock(bool use_lock)
   {
      ...
      return scoped_lock<rmutex>(hexerboost::move(local));
   }
...
}} //namespace hexerboost {} namespace boost = hexerboost; namespace hexerboost { namespace interprocess
