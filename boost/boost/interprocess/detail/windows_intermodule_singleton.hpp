...
class windows_semaphore_based_map
{
   ...
   windows_semaphore_based_map()
   {
      ...
      hexerboost::uint32_t initial_count = 0;
      hexerboost::uint32_t max_count = 0;
      ...
      if(sizeof(void*) == sizeof(hexerboost::uint32_t)){
         ...
         {
            void *addr;
            hexerboost::uint32_t addr_uint32;
         } caster;
         caster.addr = m;
         //memory is at least 4 byte aligned in windows
         BOOST_ASSERT((caster.addr_uint32 & hexerboost::uint32_t(3)) == 0);
         max_count = caster.addr_uint32 >> 2;
      }
      else if(sizeof(void*) == sizeof(hexerboost::uint64_t)){
         //Relying in UB with a cast through union, but all known windows compilers
         //accept this (C11 accepts this).
         union caster_union
         {
            void *addr;
            hexerboost::uint64_t addr_uint64;
         } caster;
         caster.addr = m;
         ...
         caster.addr = m;
         BOOST_ASSERT((caster.addr_uint64 & hexerboost::uint64_t(3)) == 0);
         max_count = hexerboost::uint32_t(caster.addr_uint64 >> 32);
         initial_count = hexerboost::uint32_t(caster.addr_uint64);
         initial_count = initial_count/4;
         //Make sure top two bits are zero
         BOOST_ASSERT((max_count & hexerboost::uint32_t(0xC0000000)) == 0);
         //Set quasi-top bit
         max_count |= hexerboost::uint32_t(0x40000000);
      }
      bool created = false;
   ...
   map_type &get_map_unlocked()
   {
      if(sizeof(void*) == sizeof(hexerboost::uint32_t)){
         ...
         hexerboost::uint32_t max_count(m_sem_map.limit()), initial_count(m_sem_map.value());
         //Clear quasi-top bit
         max_count &= hexerboost::uint32_t(0xBFFFFFFF);
         caster.addr_uint64 = max_count;
         caster.addr_uint64 =  caster.addr_uint64 << 32;
         caster.addr_uint64 |= hexerboost::uint64_t(initial_count) << 2;
         return *static_cast<map_type*>(caster.addr);
      }
   }
