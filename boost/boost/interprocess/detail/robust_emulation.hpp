...
{
   public:
   static const hexerboost::uint32_t correct_state = 0;
   static const hexerboost::uint32_t fixing_state  = 1;
   static const hexerboost::uint32_t broken_state  = 2;
   ...
   void lock();
   bool try_lock();
   bool timed_lock(const hexerboost::posix_time::ptime &abs_time);
   void unlock();
   void consistent();
   ...
   bool is_owner_dead(hexerboost::uint32_t own);
   void owner_to_filename(hexerboost::uint32_t own, std::string &s);
   //The real mutex
   Mutex mtx;
   //The pid of the owner
   volatile hexerboost::uint32_t owner;
   //The state of the mutex (correct, fixing, broken)
   volatile hexerboost::uint32_t state;
};
...
template<class Mutex>
inline void robust_spin_mutex<Mutex>::owner_to_filename(hexerboost::uint32_t own, std::string &s)
{
...
{
   hexerboost::uint32_t cur_owner = get_current_process_id();
   hexerboost::uint32_t old_owner = atomic_read32(&this->owner), old_owner2;
...
template<class Mutex>
inline bool robust_spin_mutex<Mutex>::is_owner_dead(hexerboost::uint32_t own)
{
   //If owner is an invalid id, then it's clear it's dead
   if(own == (hexerboost::uint32_t)get_invalid_process_id()){
      return true;
   }
...
inline void robust_spin_mutex<Mutex>::consistent()
{
   ...
   if(atomic_read32(&this->state) != fixing_state &&
      atomic_read32(&this->owner) != (hexerboost::uint32_t)get_current_process_id()){
      throw interprocess_exception(lock_error, "Broken id");
   }
