   ...
   : public managed_open_or_create_impl_device_holder<StoreDevice, DeviceAbstraction>
{
   ...
   static const std::size_t
      ManagedOpenOrCreateUserOffset =
         ct_rounded_size
            < sizeof(hexerboost::uint32_t)
            , MemAlignment ? (MemAlignment) :
               (::hexerboost::alignment_of< ::hexerboost::detail::max_align >::value)
            >::value;
   ...
   {
      managed_open_or_create_impl tmp(hexerboost::move(moved));
      this->swap(tmp);
      return *this;
   }
       ...
       ConstructFunc construct_func)
   {
      ...
      if(created){
         try{
            ...
            hexerboost::uint32_t *patomic_word = 0;  //avoid gcc warning
            patomic_word = static_cast<hexerboost::uint32_t*>(region.get_address());
            hexerboost::uint32_t previous = atomic_cas32(patomic_word, InitializingSegment, UninitializedSegment);
         ...
         hexerboost::uint32_t *patomic_word = static_cast<hexerboost::uint32_t*>(region.get_address());
         hexerboost::uint32_t value = atomic_read32(patomic_word);
      ...
      if(StoreDevice){
         this->DevHolder::get_device() = hexerboost::move(dev);
      }
   }
