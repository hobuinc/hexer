...
class xsi_shared_memory_device
{
   ...
   xsi_shared_memory_device(create_only_t, const char *filepath, hexerboost::uint8_t id, mode_t mode, std::size_t size)
   {  this->priv_open_or_create_name_id(ipcdetail::DoCreate, name, id, mode, size);  }

   xsi_shared_memory_device(open_or_create_t, const char *filepath, hexerboost::uint8_t id, mode_t mode, std::size_t size)
   {  this->priv_open_or_create_name_id(ipcdetail::DoOpenOrCreate, id, name, mode, size);  }

   xsi_shared_memory_device(open_only_t, const char *filepath, hexerboost::uint8_t id, mode_t mode)
   ...
   {
      xsi_shared_memory_device tmp(hexerboost::move(moved));
      this->swap(tmp);
      return *this;
   }
   ...
   (mapped_region &reg, xsi_named_mutex &mut, std::string &path)
{
   ...
   reg = hexerboost::move(r);
   mut = hexerboost::move(m);
