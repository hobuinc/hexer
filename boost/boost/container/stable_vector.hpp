...
class iterator
   : public std::iterator< std::random_access_iterator_tag
                         , T
                         , typename hexerboost::intrusive::
                              pointer_traits<Pointer>::difference_type
                         , Pointer
                         , Reference>
{
   typedef hexerboost::intrusive::
...
struct index_traits
{
   ...
   typedef ::hexerboost::container::vector
...
class stable_vector
{
   ...
   typedef ::hexerboost::container::container_detail::integral_constant
      <unsigned, hexerboost::container::container_detail::
   ...
   stable_vector(BOOST_RV_REF(stable_vector) x)
      : internal_data(hexerboost::move(x.priv_node_alloc())), index(hexerboost::move(x.index))
   {
   ...
   stable_vector& operator=(BOOST_RV_REF(stable_vector) x)
   {
      if (&x != this){
         ...
         if(this_alloc == x_alloc){
            ...
            this->clear();
            this->index = hexerboost::move(x.index);
            this->priv_swap_members(x);
   ...
   void emplace_back(Args &&...args)
   {
      ...
      EmplaceFunctor &&ef = EmplaceFunctor(hexerboost::forward<Args>(args)...);
   ...
   void priv_push_back(BOOST_MOVE_CATCH_FWD(U) x)
   {
      if(this->priv_capacity_bigger_than_size()){
         ...
         BOOST_ASSERT(!!p);
         {
            ...
            //This might throw
            this->priv_build_node_from_convertible(p, ::hexerboost::forward<U>(x));
            rollback.release();
         }
         ...
         this->insert(this->cend(), ::hexerboost::forward<U>(x));
      }
   }
   ...
   iterator priv_insert(const_iterator position, BOOST_RV_REF(T) x)
   {
      ...
      typedef hexerboost::move_iterator<repeat_it>      repeat_move_it;
      //Just call more general insert(pos, size, value) and return iterator
   ...
   {
      //This can throw
      hexerboost::container::construct_in_place
   ...
   {
      //This can throw
      hexerboost::container::allocator_traits<node_allocator_type>::construct
         ( this->priv_node_alloc()
         , container_detail::addressof(p->value)
         , ::hexerboost::forward<ValueConvertible>(value_convertible));
      //This does not throw
      ...
      : public node_allocator_type
   {
      ...
      explicit ebo_holder(BOOST_FWD_REF(AllocatorRLValue) a)
         : node_allocator_type(hexerboost::forward<AllocatorRLValue>(a))
         , pool_size(0)
         , end_node()
      {}
...
template <class T, class Allocator>
struct has_trivial_destructor_after_move<hexerboost::container::stable_vector<T, Allocator> >
