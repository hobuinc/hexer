...
class deque_base
{
 ...
 public:
   ...
   explicit deque_base(BOOST_RV_REF(deque_base) x)
      :  members_( hexerboost::move(x.ptr_alloc())
                 , hexerboost::move(x.alloc()) )
   {}
...
//      }
      ...
      ,  public allocator_type
   {
      ...
      members_holder(BOOST_FWD_REF(PtrAllocConvertible) pa, BOOST_FWD_REF(ValAllocConvertible) va)
         : map_allocator_type(hexerboost::forward<PtrAllocConvertible>(pa))
         , allocator_type    (hexerboost::forward<ValAllocConvertible>(va))
         , m_map(0), m_map_size(0)
         , m_start(), m_finish(m_start)
      {}
...
class deque : protected deque_base<T, Allocator>
{
      ...
      :  Base(allocator_traits_type::select_on_container_copy_construction(x.alloc()))
   {
      if(x.size()){
         ...
         hexerboost::container::uninitialized_copy_alloc
   ...
   deque(BOOST_RV_REF(deque) x)
      :  Base(hexerboost::move(static_cast<Base&>(x)))
   {  this->swap_members(x);   }
   ...
   deque& operator= (BOOST_RV_REF(deque) x)
   {
      if (&x != this){
         ...
         //If unequal allocators, then do a one by one move
            ...
            this->assign( hexerboost::make_move_iterator(x.begin())
                        , hexerboost::make_move_iterator(x.end()));
         }
      }
      return *this;
   }
      ...
      )
   {
      ...
      if (len > size()) {
         ...
         hexerboost::copy_or_move(first, mid, begin());
         this->insert(this->cend(), mid, last);
      }
      else{
         this->erase(hexerboost::copy_or_move(first, last, this->begin()), cend());
      }
   }
   #endif
   ...
   void emplace_front(Args&&... args)
   {
      if(this->priv_push_front_simple_available()){
         allocator_traits_type::construct
            ...
            , hexerboost::forward<Args>(args)...);
         this->priv_push_front_simple_commit();
      }
      else{
         typedef container_detail::insert_non_movable_emplace_proxy<Allocator, iterator, Args...> type;
         this->priv_insert_front_aux_impl(1, type(this->alloc(), hexerboost::forward<Args>(args)...));
      }
   }
   ...
   void emplace_back(Args&&... args)
   {
      if(this->priv_push_back_simple_available()){
         ...
         typedef container_detail::insert_non_movable_emplace_proxy<Allocator, iterator, Args...> type;
         this->priv_insert_back_aux_impl(1, type(this->alloc(), hexerboost::forward<Args>(args)...));
      }
   }
   ...
   {
      if(p == this->cbegin()){
         this->emplace_front(hexerboost::forward<Args>(args)...);
         return this->begin();
      }
      else if(p == this->cend()){
         this->emplace_back(hexerboost::forward<Args>(args)...);
         return (this->end()-1);
      }
      else{
         typedef container_detail::insert_emplace_proxy<Allocator, iterator, Args...> type;
         return this->priv_insert_aux_impl(p, 1, type(this->alloc(), hexerboost::forward<Args>(args)...));
      }
   }
   ...
   iterator erase(const_iterator pos) BOOST_CONTAINER_NOEXCEPT
   {
      ...
      if (index < (this->size()/2)) {
         hexerboost::move_backward(begin(), iterator(pos), iterator(next));
         pop_front();
      }
      else {
         hexerboost::move(iterator(next), end(), iterator(pos));
         pop_back();
      }
   ...
   iterator erase(const_iterator first, const_iterator last) BOOST_CONTAINER_NOEXCEPT
   {
      if (first == this->members_.m_start && last == this->members_.m_finish) {
         ...
         if (elems_before < (this->size() - n) - elems_before) {
            hexerboost::move_backward(begin(), iterator(first), iterator(last));
            ...
            hexerboost::move(iterator(last), end(), iterator(first));
   ...
   {
      if (position == cbegin()){
         this->push_front(::hexerboost::forward<U>(x));
         return begin();
      }
      else if (position == cend()){
         this->push_back(::hexerboost::forward<U>(x));
         return --end();
      }
      else {
         return priv_insert_aux_impl
            (position, (size_type)1, container_detail::get_insert_value_proxy<iterator>(this->alloc(), ::hexerboost::forward<U>(x)));
      }
   }
   ...
   void priv_push_front(BOOST_FWD_REF(U) x)
   {
      if(this->priv_push_front_simple_available()){
         allocator_traits_type::construct
            ( this->alloc(), this->priv_push_front_simple_pos(), ::hexerboost::forward<U>(x));
         this->priv_push_front_simple_commit();
      }
      else{
         priv_insert_aux_impl
            (this->cbegin(), (size_type)1, container_detail::get_insert_value_proxy<iterator>(this->alloc(), ::hexerboost::forward<U>(x)));
      }
   }
   ...
   void priv_push_back(BOOST_FWD_REF(U) x)
   {
      if(this->priv_push_back_simple_available()){
         allocator_traits_type::construct
            ( this->alloc(), this->priv_push_back_simple_pos(), ::hexerboost::forward<U>(x));
         this->priv_push_back_simple_commit();
      }
      else{
         priv_insert_aux_impl
            (this->cend(), (size_type)1, container_detail::get_insert_value_proxy<iterator>(this->alloc(), ::hexerboost::forward<U>(x)));
      ...
      if (elemsbefore < length / 2) {
         ...
         if(!elemsbefore){
            ...
            if (elemsbefore >= n) {
               ...
               this->members_.m_start = new_start;
               hexerboost::move(start_n, pos, old_start);
   ...
   void priv_fill_initialize(const value_type& value)
   {
      ...
      BOOST_TRY {
         for (cur = this->members_.m_start.m_node; cur < this->members_.m_finish.m_node; ++cur){
            hexerboost::container::uninitialized_fill_alloc
   ...
   void priv_range_initialize(FwdIt first, FwdIt last, std::forward_iterator_tag)
   {
      ...
      BOOST_TRY {
         for (cur_node = this->members_.m_start.m_node;
            ...
            ::hexerboost::container::uninitialized_copy_or_move_alloc
   ...
   void priv_reallocate_map(size_type nodes_to_add, bool add_at_front)
   {
      ...
      if (this->members_.m_map_size > 2 * new_num_nodes) {
         ...
         if (new_nstart < this->members_.m_start.m_node)
            hexerboost::move(this->members_.m_start.m_node, this->members_.m_finish.m_node + 1, new_nstart);
         else
            hexerboost::move_backward
...
template <class T, class Allocator>
struct has_trivial_destructor_after_move<hexerboost::container::deque<T, Allocator> >
