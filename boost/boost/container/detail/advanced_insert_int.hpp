...
namespace hexerboost {} namespace boost = hexerboost; namespace hexerboost { namespace container { namespace container_detail {
...
struct insert_range_proxy
{
   ...
   {
      this->first_ = ::hexerboost::container::uninitialized_copy_or_move_alloc_n_source
         (this->a_, this->first_, n, pos);
   }

   void copy_n_and_update(Iterator pos, size_type n)
   {
      this->first_ = ::hexerboost::container::copy_or_move_n_source(this->first_, n, pos);
   }
...
{
   typedef ::hexerboost::container::allocator_traits<A> alloc_traits;
   typedef typename allocator_traits<A>::size_type size_type;
   typedef typename allocator_traits<A>::value_type value_type;
...
struct insert_non_movable_emplace_proxy
{
   ...
   void priv_uninitialized_copy_some_and_update(const index_tuple<IdxPack...>&, Iterator p, size_type n)
   {
      ...
      alloc_traits::construct( this->a_
                              , container_detail::to_raw_pointer(&*p)
                              , ::hexerboost::forward<Args>(get<IdxPack>(this->args_))...
                              );
   }
   ...
   :  public insert_non_movable_emplace_proxy<A, Iterator, Args...>
{
   ...
   explicit insert_emplace_proxy(A &a, Args&&... args)
      : base_t(a, ::hexerboost::forward<Args>(args)...)
   {}
