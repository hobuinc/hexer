...
class map
{
   ...
   map(BOOST_RV_REF(map) x)
      : m_tree(hexerboost::move(x.m_tree))
   {
      //Allocator type must be std::pair<CONST Key, T>
      BOOST_STATIC_ASSERT((container_detail::is_same<std::pair<const Key, T>, typename Allocator::value_type>::value));
   }
   ...
   map(BOOST_RV_REF(map) x, const allocator_type &a)
      : m_tree(hexerboost::move(x.m_tree), a)
   {
      //Allocator type must be std::pair<CONST Key, T>
      BOOST_STATIC_ASSERT((container_detail::is_same<std::pair<const Key, T>, typename Allocator::value_type>::value));
   }
   ...
   {  m_tree = hexerboost::move(x.m_tree);   return *this;  }
   ...
   //! Effects: If there is no key equivalent to x in the map, inserts
   //! value_type(hexerboost::move(x), T()) into the map (the key is move-constructed)
   //!
   //! Returns: Allocator reference to the mapped_type corresponding to x in *this.
   //!
   //! Complexity: Logarithmic.
   ...
   { return m_tree.insert_unique(hexerboost::move(x)); }
   ...
   { return m_tree.insert_unique(position, hexerboost::move(x)); }
   ...
   {  return m_tree.emplace_unique(hexerboost::forward<Args>(args)...); }
   ...
   {  return m_tree.emplace_hint_unique(hint, hexerboost::forward<Args>(args)...); }
   ...
   mapped_type& priv_subscript(const key_type &k)
   {
      ...
      if (i == end() || key_comp()(k, (*i).first)){
         ...
         movable_value_type val(k, hexerboost::move(m.m_t));
         i = insert(i, hexerboost::move(val));
      }
      return (*i).second;
   }
   ...
   mapped_type& priv_subscript(BOOST_RV_REF(key_type) mk)
   {
      ...
      if (i == end() || key_comp()(k, (*i).first)){
         container_detail::value_init<mapped_type> m;
         movable_value_type val(hexerboost::move(k), hexerboost::move(m.m_t));
         i = insert(i, hexerboost::move(val));
      }
      return (*i).second;
   }
...
struct has_trivial_destructor_after_move<hexerboost::container::map<K, T, C, Allocator> >
{
...
class multimap
{
   ...
   {  return m_tree.emplace_equal(hexerboost::forward<Args>(args)...); }
   ...
   template <class... Args>
   iterator emplace_hint(const_iterator hint, Args&&... args)
   {  return m_tree.emplace_hint_equal(hint, hexerboost::forward<Args>(args)...); }
   ...
   { return m_tree.insert_equal(hexerboost::move(x)); }
   ...
   //!   is inserted right before p.
   iterator insert(const_iterator position, BOOST_RV_REF(nonconst_value_type) x)
   { return m_tree.insert_equal(position, hexerboost::move(x)); }
...
//!specialization for optimizations
template <class K, class T, class C, class Allocator>
struct has_trivial_destructor_after_move<hexerboost::container::multimap<K, T, C, Allocator> >
{
   static const bool value = has_trivial_destructor_after_move<Allocator>::value && has_trivial_destructor_after_move<C>::value;
};
