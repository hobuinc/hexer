...
namespace hexerboost {} namespace boost = hexerboost; namespace hexerboost { namespace program_options { 
    ...
    template<class T, class charT>
    void 
    typed_value<T, charT>::notify(const hexerboost::any& value_store) const
    {
        const T* value = hexerboost::any_cast<T>(&value_store);
        if (m_store_to) {
            *m_store_to = *value;
        }
        if (m_notifier) {
    ...
    namespace validators {
            ...
            bool allow_empty = false)
        {
            ...
            if (v.size() > 1)
                hexerboost::throw_exception(validation_error(validation_error::multiple_values_not_allowed));
            else if (v.size() == 1)
                return v.front();
            else if (!allow_empty)
                hexerboost::throw_exception(validation_error(validation_error::at_least_one_value_required));
            return empty;
        }

        /* Throws multiple_occurrences if 'value' is not empty. */
        BOOST_PROGRAM_OPTIONS_DECL void 
        check_first_occurrence(const hexerboost::any& value);
    }
    ...
    */
    template<class T, class charT>
    void validate(hexerboost::any& v, 
                  ...
                  T*, long)
    {
        ...
        try {
            ...
            hexerboost::throw_exception(invalid_option_value(s));
        }
    }

    BOOST_PROGRAM_OPTIONS_DECL void validate(hexerboost::any& v, 
    ...
    {
        if (v.empty()) {
            v = hexerboost::any(std::vector<T>());
        }
        std::vector<T>* tv = hexerboost::any_cast< std::vector<T> >(&v);
        assert(NULL != tv);
        for (unsigned i = 0; i < s.size(); ++i)
        {
            try {
                ...
                cv.push_back(s[i]);
                validate(a, cv, (T*)0, 0);                
                tv->push_back(hexerboost::any_cast<T>(a));
            }
            catch(const bad_lexical_cast& /*e*/) {
                hexerboost::throw_exception(invalid_option_value(s[i]));
            }
        }
    }
    ...
    void 
    typed_value<T, charT>::
    xparse(hexerboost::any& value_store, 
    ...
    value()
    {
        ...
        return hexerboost::program_options::value<T>(0);
    }
