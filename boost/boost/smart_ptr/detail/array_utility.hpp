...
namespace hexerboost {} namespace boost = hexerboost; namespace hexerboost {
    namespace detail {
        template<typename T>
        inline void array_destroy(T*, std::size_t, hexerboost::true_type) {
        }
        template<typename T>
        inline void array_destroy(T* memory, std::size_t size, hexerboost::false_type) {
        ...
        inline void array_destroy(T* memory, std::size_t size) {
            hexerboost::has_trivial_destructor<T> type;
            array_destroy(memory, size, type);
        }
        template<typename T>
        inline void array_init(T* memory, std::size_t size, hexerboost::true_type) {
            for (std::size_t i = 0; i < size; i++) {
                memory[i] = T();
            }
        }
        template<typename T>
        inline void array_init(T* memory, std::size_t size, hexerboost::false_type) {
        ...
        inline void array_init(T* memory, std::size_t size) {
            hexerboost::has_trivial_default_constructor<T> type;            
        ...
        template<typename T>
        inline void array_noinit(T*, std::size_t, hexerboost::true_type) {
        }
        template<typename T>
        inline void array_noinit(T* memory, std::size_t size, hexerboost::false_type) {
#if !defined(BOOST_NO_EXCEPTIONS)
            std::size_t i = 0;
            try {
                for (; i < size; i++) {
