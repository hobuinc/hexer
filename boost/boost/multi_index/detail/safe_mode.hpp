...
inline void detach_equivalent_iterators(Iterator& it)
{
  if(it.valid()){
    {
#if defined(BOOST_HAS_THREADS)
      hexerboost::detail::lightweight_mutex::scoped_lock lock(it.cont->mutex);
#endif
...
#if defined(BOOST_HAS_THREADS)
  hexerboost::detail::lightweight_mutex mutex;
#endif
};
...
void safe_iterator_base::attach(safe_container_base* cont_)
{
  ...
  if(cont){
#if defined(BOOST_HAS_THREADS)
    hexerboost::detail::lightweight_mutex::scoped_lock lock(cont->mutex);
#endif
...
private:
  friend class hexerboost::multi_index::detail::iter_adaptor_access;
