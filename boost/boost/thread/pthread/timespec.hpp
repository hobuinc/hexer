...
namespace hexerboost {} namespace boost = hexerboost; namespace hexerboost
{
  ...
  {
#if defined BOOST_THREAD_USES_DATETIME
    inline struct timespec to_timespec(hexerboost::system_time const& abs_time)
    {
      struct timespec timeout = { 0,0};
      hexerboost::posix_time::time_duration const time_since_epoch=abs_time-hexerboost::posix_time::from_time_t(0);
    ...
    inline timespec to_timespec(hexerboost::intmax_t const& ns)
    {
      hexerboost::intmax_t s = ns / 1000000000l;
      struct timespec ts;
      ts.tv_sec = static_cast<long> (s);
      ts.tv_nsec = static_cast<long> (ns - s * 1000000000l);
      return ts;
    }
    inline hexerboost::intmax_t to_nanoseconds_int_max(timespec const& ts)
    {
      return static_cast<hexerboost::intmax_t>(ts.tv_sec) * 1000000000l + ts.tv_nsec;
    }
