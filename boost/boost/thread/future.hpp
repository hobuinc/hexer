...
// hexerboost::thread::future requires exception handling
// due to hexerboost::exception::exception_ptr dependency
...
namespace hexerboost {} namespace boost = hexerboost; namespace hexerboost
{
    ...
    namespace detail
    {
        ...
        struct future_continuation_base
        {
          ...
          virtual void do_continuation(hexerboost::unique_lock<hexerboost::mutex>& ) {};
        private:
        ...
        struct relocker
        {
            hexerboost::unique_lock<hexerboost::mutex>& lock_;
            bool  unlocked_;

            relocker(hexerboost::unique_lock<hexerboost::mutex>& lk):
                lock_(lk)
            {
                lock_.unlock();
                unlocked_=true;
        ...
        struct future_object_base : enable_shared_from_this<future_object_base>
        {
            ...
            hexerboost::exception_ptr exception;
            bool done;
            bool is_deferred_;
            launch policy_;
            bool is_constructed;
            hexerboost::mutex mutex;
            hexerboost::condition_variable waiters;
            typedef std::list<hexerboost::condition_variable_any*> waiter_list;
            waiter_list external_waiters;
            hexerboost::function<void()> callback;
...
//#endif
            ...
            waiter_list::iterator register_external_waiter(hexerboost::condition_variable_any& cv)
            {
                hexerboost::unique_lock<hexerboost::mutex> lock(mutex);
                do_callback(lock);
            ...
            {
                hexerboost::lock_guard<hexerboost::mutex> lock(mutex);
                external_waiters.erase(it);
            }

#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
            void do_continuation(hexerboost::unique_lock<hexerboost::mutex>& lock)
            {
                if (continuation_ptr) {
...
#else
            void do_continuation(hexerboost::unique_lock<hexerboost::mutex>&)
            {
            }
#endif
#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
            void set_continuation_ptr(future_continuation_base* continuation, hexerboost::unique_lock<hexerboost::mutex>& lock)
            {
...
#endif
            void mark_finished_internal(hexerboost::unique_lock<hexerboost::mutex>& lock)
            {
                done=true;
                waiters.notify_all();
            ...
            void do_callback(hexerboost::unique_lock<hexerboost::mutex>& lock)
            {
                if(callback && !done)
                {
                    hexerboost::function<void()> local_callback=callback;
            ...
            void wait_internal(hexerboost::unique_lock<hexerboost::mutex> &lock, bool rethrow=true)
            {
              do_callback(lock);
              //if (!done) // fixme why this doesn't works?
              {
                ...
                else
                {
                  ...
                  {
                      throw hexerboost::thread_interrupted();
                  }
#endif
                  if(rethrow && exception)
                  {
                      hexerboost::rethrow_exception(exception);
                  }
                }
              }
            }
            ...
            bool timed_wait_until(hexerboost::system_time const& target_time)
            {
                hexerboost::unique_lock<hexerboost::mutex> lock(mutex);
                if (is_deferred_)
                    return false;
...
#endif
            void mark_exceptional_finish_internal(hexerboost::exception_ptr const& e, hexerboost::unique_lock<hexerboost::mutex>& lock)
            {
                exception=e;
                mark_finished_internal(lock);
            }
            ...
            {
                hexerboost::unique_lock<hexerboost::mutex> lock(mutex);
                mark_exceptional_finish_internal(hexerboost::current_exception(), lock);
            }
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
            ...
            void set_interrupted_hexerboostat_thread_exit()
            {
              unique_lock<hexerboost::mutex> lk(mutex);
              thread_was_interrupted=true;
              if (has_value(lk))
              {
                  throw_exception(promise_already_satisfied());
              }
              get_current_thread_data()->make_ready_hexerboostat_thread_exit(shared_from_this());
            }
#endif
            void set_exception_hexerboostat_thread_exit(exception_ptr e)
            {
              unique_lock<hexerboost::mutex> lk(mutex);
              if (has_value(lk))
              {
            ...
            bool has_value(unique_lock<hexerboost::mutex>& )
            {
                return done && !(exception
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
                    || thread_was_interrupted
            ...
            bool has_exception(unique_lock<hexerboost::mutex>&)
            {
                return done && (exception
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
                    || thread_was_interrupted
            ...
            {
                hexerboost::lock_guard<hexerboost::mutex> lock(mutex);
                callback=hexerboost::bind(f,hexerboost::ref(*u));
            }
            virtual void execute(hexerboost::unique_lock<hexerboost::mutex>&) {}
        ...
        {
            typedef hexerboost::scoped_ptr<T> storage_type;
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
            typedef T const& source_reference_type;
            struct dummy;
            typedef typename hexerboost::mpl::if_<hexerboost::is_fundamental<T>,dummy&,BOOST_THREAD_RV_REF(T)>::type rvalue_source_type;
            typedef typename hexerboost::mpl::if_<hexerboost::is_fundamental<T>,T,BOOST_THREAD_RV_REF(T)>::type move_dest_type;
#elif defined BOOST_THREAD_USES_MOVE
            typedef T& source_reference_type;
            typedef typename hexerboost::mpl::if_<hexerboost::has_move_emulation_enabled<T>,BOOST_THREAD_RV_REF(T),T const&>::type rvalue_source_type;
            typedef typename hexerboost::mpl::if_<hexerboost::has_move_emulation_enabled<T>,BOOST_THREAD_RV_REF(T),T>::type move_dest_type;
#else
            typedef T& source_reference_type;
            typedef typename hexerboost::mpl::if_<hexerboost::thread_detail::is_convertible<T&,BOOST_THREAD_RV_REF(T) >,BOOST_THREAD_RV_REF(T),T const&>::type rvalue_source_type;
            typedef typename hexerboost::mpl::if_<hexerboost::thread_detail::is_convertible<T&,BOOST_THREAD_RV_REF(T) >,BOOST_THREAD_RV_REF(T),T>::type move_dest_type;
#endif
            ...
            detail::future_object_base
        {
            ...
            void mark_finished_with_result_internal(source_reference_type result_, hexerboost::unique_lock<hexerboost::mutex>& lock)
            {
                future_traits<T>::init(result,result_);
                mark_finished_internal(lock);
            }

            void mark_finished_with_result_internal(rvalue_source_type result_, hexerboost::unique_lock<hexerboost::mutex>& lock)
            {
            ...
            {
                hexerboost::lock_guard<hexerboost::mutex> guard(mutex);
                if(!done)
                {
            ...
            void set_value_hexerboostat_thread_exit(const T & result_)
            {
              unique_lock<hexerboost::mutex> lk(this->mutex);
            ...
            void set_value_hexerboostat_thread_exit(BOOST_THREAD_RV_REF(T) result_)
            {
              unique_lock<hexerboost::mutex> lk(this->mutex);
              if (this->has_value(lk))
                  throw_exception(promise_already_satisfied());
              result.reset(new T(hexerboost::move(result_)));
              //future_traits<T>::init(result,static_cast<rvalue_source_type>(result_));
              this->is_constructed = true;
              get_current_thread_data()->make_ready_hexerboostat_thread_exit(shared_from_this());
            }
            ...
            detail::future_object_base
        {
            ...
            void mark_finished_with_result_internal(T& result_, hexerboost::unique_lock<hexerboost::mutex>& lock)
            {
                //future_traits<T>::init(result,result_);
                result= &result_;
                mark_finished_internal(lock);
...
//            }
            ...
            void set_value_hexerboostat_thread_exit(T& result_)
            {
              unique_lock<hexerboost::mutex> lk(this->mutex);
              if (this->has_value(lk))
                  throw_exception(promise_already_satisfied());
...
//            void set_value_hexerboostat_thread_exit(rvalue_source_type result_)
//            {
//              unique_lock<hexerboost::mutex> lk(this->mutex);
//              if (this->has_value())
//                  throw_exception(promise_already_satisfied());
...
//            }
            ...
            detail::future_object_base
        {
          typedef void shared_future_get_result_type;
            ...
            void mark_finished_with_result_internal(hexerboost::unique_lock<hexerboost::mutex>& lock)
            {
                mark_finished_internal(lock);
            }
            ...
            void set_value_hexerboostat_thread_exit()
            {
              unique_lock<hexerboost::mutex> lk(this->mutex);
              if (this->has_value(lk))
              {
        ...
        struct future_async_object: future_object<Rp>
        {
          ...
          hexerboost::thread thr_;

        public:
          explicit future_async_object(BOOST_THREAD_FWD_REF(Fp) f) :
            thr_(&future_async_object::run, this, hexerboost::forward<Fp>(f))
          {
            this->set_async();
          }
          ...
          move_dest_type get()
          {
              ...
              //this->wait();
              hexerboost::unique_lock<hexerboost::mutex> lock(this->mutex);
        ...
        public:
          ...
          : func_(hexerboost::forward<Fp>(f))
          {
            this->set_deferred();
          }

          virtual void execute(hexerboost::unique_lock<hexerboost::mutex>& lck) {
            try
            {
...
//        };
        class future_waiter
        {
            ...
            struct registered_waiter
            {
                hexerboost::shared_ptr<detail::future_object_base> future_;
                detail::future_object_base::waiter_list::iterator wait_iterator;
                count_type index;

                registered_waiter(hexerboost::shared_ptr<detail::future_object_base> const& a_future,
            ...
            struct all_futures_lock
            {
                   ...
                   hexerboost::scoped_array<hexerboost::unique_lock<hexerboost::mutex> > locks;

                all_futures_lock(std::vector<registered_waiter>& futures):
                    count(futures.size()),locks(new hexerboost::unique_lock<hexerboost::mutex>[count])
                {
                    for(count_type_portable i=0;i<count;++i)
                    {
#if defined __DECCXX || defined __SUNPRO_CC || defined __hpux
                        locks[i]=hexerboost::unique_lock<hexerboost::mutex>(futures[i].future_->mutex).move();
#else
                        locks[i]=hexerboost::unique_lock<hexerboost::mutex>(futures[i].future_->mutex); // TODO shouldn't be moved explicitly
#endif
                    }
                }
                ...
                void lock()
                {
                    hexerboost::lock(locks.get(),locks.get()+count);
                }
            ...
            hexerboost::condition_variable_any cv;
    ...
    typename hexerboost::disable_if<is_future_type<Iterator>,void>::type wait_for_all(Iterator begin,Iterator end)
    {
    ...
    typename hexerboost::enable_if<is_future_type<F1>,void>::type wait_for_all(F1& f1,F2& f2)
    {
        f1.wait();
        f2.wait();
    }
    ...
    template<typename Iterator>
    typename hexerboost::disable_if<is_future_type<Iterator>,Iterator>::type wait_for_any(Iterator begin,Iterator end)
    {
        if(begin==end)
            return end;
        ...
        return hexerboost::next(begin,waiter.wait());
    }

    template<typename F1,typename F2>
    typename hexerboost::enable_if<is_future_type<F1>,unsigned>::type wait_for_any(F1& f1,F2& f2)
    {
        detail::future_waiter waiter;
        waiter.add(f1);
        waiter.add(f2);
    ...
    namespace detail
    {
      ...
      protected:
        ...
        typedef hexerboost::shared_ptr<detail::future_object<R> > future_ptr;
      ...
      public:
        ...
        void wait() const
        {
            if(!future_)
            {
                hexerboost::throw_exception(future_uninitialized());
            }
            future_->wait(false);
        }
        ...
        {
            return timed_wait_until(hexerboost::get_system_time()+rel_time);
        }

        bool timed_wait_until(hexerboost::system_time const& abs_time) const
        {
            if(!future_)
            {
                hexerboost::throw_exception(future_uninitialized());
    ...
    public:
        ...
        base_type(hexerboost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))))
        {
        }

        BOOST_THREAD_FUTURE& operator=(BOOST_THREAD_RV_REF(BOOST_THREAD_FUTURE) other) BOOST_NOEXCEPT
        {
            base_type::operator=(hexerboost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))));
            return *this;
        }

        shared_future<R> share()
        {
          return shared_future<R>(::hexerboost::move(*this));
        }
...
//        auto then(F&& func) -> BOOST_THREAD_FUTURE<decltype(func(*this))>;
...
#endif
        template<typename F>
        inline BOOST_THREAD_FUTURE<typename hexerboost::result_of<F(BOOST_THREAD_FUTURE&)>::type>
    ...
    public:
        ...
        {
            base_type::operator=(hexerboost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))));
            //shared_future(hexerboost::move(other)).swap(*this);
    ...
    public:
        ...
        promise(hexerboost::allocator_arg_t, Allocator a)
        {
        ...
        ~promise()
        {
            if(future_)
            {
                hexerboost::unique_lock<hexerboost::mutex> lock(future_->mutex);

                if(!future_->done)
                {
                    future_->mark_exceptional_finish_internal(hexerboost::copy_exception(broken_promise()), lock);
                }
            }
        }
        ...
        BOOST_THREAD_FUTURE<R> get_future()
        {
            ...
            if (future_.get()==0)
            {
                hexerboost::throw_exception(promise_moved());
            }
            if (future_obtained)
            {
                hexerboost::throw_exception(future_already_retrieved());
            }
        ...
        void set_value(typename detail::future_traits<R>::source_reference_type r)
        {
            ...
            if(future_->done)
            {
                hexerboost::throw_exception(promise_already_satisfied());
            }
...
//         void set_value(R && r);
        ...
        void set_exception(hexerboost::exception_ptr p)
        {
            lazy_init();
            hexerboost::unique_lock<hexerboost::mutex> lock(future_->mutex);
            if(future_->done)
        ...
        void set_value_hexerboostat_thread_exit(const R& r)
        {
          if (future_.get()==0)
          {
              hexerboost::throw_exception(promise_moved());
          }
          future_->set_value_hexerboostat_thread_exit(r);
        }

        void set_value_hexerboostat_thread_exit(BOOST_THREAD_RV_REF(R) r)
        {
          if (future_.get()==0)
          {
              hexerboost::throw_exception(promise_moved());
          }
          future_->set_value_hexerboostat_thread_exit(hexerboost::move(r));
        }
        void set_exception_hexerboostat_thread_exit(exception_ptr e)
        {
          if (future_.get()==0)
          ...
          future_->set_exception_hexerboostat_thread_exit(e);
        }
    ...
    public:
        ...
        // setting the result with deferred notification
        void set_value_hexerboostat_thread_exit(R& r)
        {
          if (future_.get()==0)
          {
              hexerboost::throw_exception(promise_moved());
    ...
    class promise<void>
    {
        typedef hexerboost::shared_ptr<detail::future_object<void> > future_ptr;
    ...
    public:
        ...
        void set_value_hexerboostat_thread_exit()
        {
          ...
          future_->set_value_hexerboostat_thread_exit();
        }
    ...
    namespace detail
    {
      ...
      struct task_base:
            ...
            detail::future_object<R>
        {
            ...
            {
                {
                    hexerboost::lock_guard<hexerboost::mutex> lk(this->mutex);
                    if(started)
                    {
                        hexerboost::throw_exception(task_already_started());
                    }
                    started=true;
                }
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
                do_run(hexerboost::forward<ArgTypes>(args)...);
#else
                do_run();
#endif
            }
...
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
                do_apply(hexerboost::forward<ArgTypes>(args)...);
#else
                do_apply();
#endif
            }
            ...
            {
                hexerboost::unique_lock<hexerboost::mutex> lk(this->mutex);
                if(!started)
                {
                    started=true;
                    this->mark_exceptional_finish_internal(hexerboost::copy_exception(hexerboost::broken_promise()), lk);
                }
            }
        ...
        public:
            ...
            {}
            task_object(BOOST_THREAD_RV_REF(F) f_):
                f(hexerboost::move(f_)) // TODO forward
            {}
#endif
            ...
            void do_apply(BOOST_THREAD_RV_REF(ArgTypes) ... args)
            {
                try
                {
                    this->set_value_hexerboostat_thread_exit(f(hexerboost::forward<ArgTypes>(args)...));
                }
#else
            void do_apply()
            {
                try
                {
                    this->set_value_hexerboostat_thread_exit(f());
                }
#endif
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
                catch(thread_interrupted& )
                {
                    this->set_interrupted_hexerboostat_thread_exit();
                }
#endif
                catch(...)
                {
                    this->set_exception_hexerboostat_thread_exit(current_exception());
                }
            }
            ...
            void do_run(BOOST_THREAD_RV_REF(ArgTypes) ... args)
            {
                try
                {
                    this->mark_finished_with_result(f(hexerboost::forward<ArgTypes>(args)...));
                }
#else
            void do_run()
            {
        ...
        public:
            ...
            void do_apply(BOOST_THREAD_RV_REF(ArgTypes) ... args)
            {
              try
              {
                f(hexerboost::forward<ArgTypes>(args)...);
#else
            void do_apply()
            {
                try
                {
                    f();
#endif
                  this->set_value_hexerboostat_thread_exit();
                }
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
                catch(thread_interrupted& )
                {
    ...
    {
      typedef hexerboost::shared_ptr<detail::task_base<R(ArgTypes...)> > task_ptr;
      hexerboost::shared_ptr<detail::task_base<R(ArgTypes...)> > task;
  #else
    template<typename R>
    class packaged_task<R()>
    {
      typedef hexerboost::shared_ptr<detail::task_base<R()> > task_ptr;
      hexerboost::shared_ptr<detail::task_base<R()> > task;
  #endif
#else
    template<typename R>
    class packaged_task
    ...
    public:
        ...
        explicit packaged_task(R(*f)(), BOOST_THREAD_FWD_REF(ArgTypes)... args)
        {
            ...
            task= task_ptr(new task_object_type(f, hexerboost::forward<ArgTypes>(args)...));
            future_obtained=false;
        }
  #else
            ...
            )
        {
          ...
          typedef F FR;
            ...
            typedef detail::task_object<FR,R> task_object_type;
#endif
            task = task_ptr(new task_object_type(hexerboost::forward<F>(f)));
            future_obtained = false;
...
#else
            typedef detail::task_object<F,R()> task_object_type;
            task = task_ptr(new task_object_type(hexerboost::move<F>(f))); // TODO forward
#endif
#else
            typedef detail::task_object<F,R> task_object_type;
            task = task_ptr(new task_object_type(hexerboost::forward<F>(f)));
...
#if defined(BOOST_THREAD_RVALUE_REFERENCES_DONT_MATCH_FUNTION_PTR)
        template <class Allocator>
        packaged_task(hexerboost::allocator_arg_t, Allocator a, R(*f)())
        {
          typedef R(*FR)();
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
  #if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
        ...
        packaged_task(hexerboost::allocator_arg_t, Allocator a, BOOST_THREAD_RV_REF(F) f)
        {
          //typedef typename remove_cv<typename remove_reference<F>::type>::type FR;
          typedef F FR;
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
          ...
          task = task_ptr(::new(a2.allocate(1)) task_object_type(hexerboost::forward<F>(f)), D(a2, 1) );
          future_obtained = false;
        }
#else // ! defined BOOST_NO_CXX11_RVALUE_REFERENCES
        template <class F, class Allocator>
        packaged_task(hexerboost::allocator_arg_t, Allocator a, const F& f)
        {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
  #if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          typedef detail::task_object<F,R(ArgTypes...)> task_object_type;
        ...
        packaged_task(hexerboost::allocator_arg_t, Allocator a, BOOST_THREAD_RV_REF(F) f)
        {
          ...
          task = task_ptr(::new(a2.allocate(1)) task_object_type(hexerboost::forward<F>(f)), D(a2, 1) );
#else
          task = task_ptr(::new(a2.allocate(1)) task_object_type(hexerboost::move(f)), D(a2, 1) );  // TODO forward
#endif
          future_obtained = false;
        }
        ...
        BOOST_THREAD_FUTURE<R> get_future()
        {
            ...
            if(!task)
            {
                hexerboost::throw_exception(task_moved());
            }
        ...
        void operator()(BOOST_THREAD_RV_REF(ArgTypes)... args)
        {
            ...
            task->run(hexerboost::forward<ArgTypes>(args)...);
        }
        void make_ready_hexerboostat_thread_exit(ArgTypes... args)
        {
          if(!task)
          {
              hexerboost::throw_exception(task_moved());
          ...
          task->apply(hexerboost::forward<ArgTypes>(args)...);
        }
#else
        void operator()()
        {
        ...
        void make_ready_hexerboostat_thread_exit()
        {
          if(!task)
          {
              hexerboost::throw_exception(task_moved());
    ...
    make_future_deferred_object(BOOST_THREAD_FWD_REF(Fp) f)
    {
      shared_ptr<future_deferred_object<Rp, Fp> >
          h(new future_deferred_object<Rp, Fp>(hexerboost::forward<Fp>(f)));
    ...
    make_future_async_object(BOOST_THREAD_FWD_REF(Fp) f)
    {
      shared_ptr<future_async_object<Rp, Fp> >
          h(new future_async_object<Rp, Fp>(hexerboost::forward<Fp>(f)));
      return BOOST_THREAD_FUTURE<Rp>(h);
    }
    ...
    ////////////////////////////////
...
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
              hexerboost::thread( hexerboost::move(pt),  hexerboost::forward<ArgTypes>(args)... ).detach();
#else
              hexerboost::thread( hexerboost::move(pt) ).detach();
#endif
              return ::hexerboost::move(ret);
            }
        ...
        template <class F, class ...ArgTypes>
        BOOST_THREAD_FUTURE<typename hexerboost::result_of<typename decay<F>::type(
            typename decay<ArgTypes>::type...
        )>::type>
        async(launch policy, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(ArgTypes)... args)
        {

          typedef typename hexerboost::result_of<typename decay<F>::type(
              typename decay<ArgTypes>::type...
          )>::type R;
          typedef packaged_task<R(ArgTypes...)> packaged_task_type;
  ...
  #else
        template <class F>
        BOOST_THREAD_FUTURE<typename hexerboost::result_of<typename decay<F>::type()>::type>
        async(launch policy, BOOST_THREAD_FWD_REF(F)  f)
        {
          typedef typename hexerboost::result_of<typename decay<F>::type()>::type R;
          typedef packaged_task<R()> packaged_task_type;
        ...
        {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          return hexerboost::detail::make_future_async_object<Rp>(
              BF(
                  thread_detail::decay_copy(hexerboost::forward<F>(f))
                  , thread_detail::decay_copy(hexerboost::forward<ArgTypes>(args))...
              )
          );
#else
          packaged_task_type pt( hexerboost::forward<F>(f) );

          BOOST_THREAD_FUTURE<R> ret = pt.get_future();
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          hexerboost::thread( hexerboost::move(pt), hexerboost::forward<ArgTypes>(args)... ).detach(); // todo forward
#else
          hexerboost::thread( hexerboost::move(pt) ).detach();
#endif
          return ::hexerboost::move(ret);
        ...
        {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          return hexerboost::detail::make_future_deferred_object<Rp>(
              BF(
                  thread_detail::decay_copy(hexerboost::forward<F>(f))
                  , thread_detail::decay_copy(hexerboost::forward<ArgTypes>(args))...
              )
...
//          );
    ...
    template <class F>
    BOOST_THREAD_FUTURE<typename hexerboost::result_of<F()>::type>
    async(BOOST_THREAD_RV_REF(F) f)
    {
        return async(launch(launch::any), hexerboost::forward<F>(f));
    }
  ...
  BOOST_THREAD_FUTURE<typename decay<T>::type> make_future(BOOST_THREAD_FWD_REF(T) value)
  {
    ...
    p.set_value(hexerboost::forward<T>(value));
    return p.get_future();
  }
  ...
  namespace detail
  {
      ...
      struct future_continuation : future_continuation_base
      {
        ...
        future_continuation(F& f, BOOST_THREAD_FWD_REF(C) c) :
          parent(f),
          continuation(hexerboost::forward<C>(c)),
        ...
        void do_continuation(hexerboost::unique_lock<hexerboost::mutex>& lk)
        {
          try
          {
            lk.unlock();
...
//            if (int(policy_) & int(launch::deferred))
            {
              ...
              next.set_value(hexerboost::move(val));
            }
//            else
//            {
//              BOOST_THREAD_FUTURE<R> f = async(policy_, continuation, hexerboost::ref(parent));
//              R val = f.get();
//              next.set_value(hexerboost::move(val));
//            }
          }
          ...
          {
            next.set_exception(hexerboost::current_exception());
          }
        }
      private:
...
//            }
  ...
  template <typename R>
  template <typename F>
  inline BOOST_THREAD_FUTURE<typename hexerboost::result_of<F(BOOST_THREAD_FUTURE<R>&)>::type>
  BOOST_THREAD_FUTURE<R>::then(launch policy, BOOST_THREAD_RV_REF(F) func)
  {

    typedef typename hexerboost::result_of<F(BOOST_THREAD_FUTURE<R>&)>::type future_type;

    if (this->future_)
    {
      hexerboost::unique_lock<hexerboost::mutex> lock(this->future_->mutex);
      detail::future_continuation<BOOST_THREAD_FUTURE<R>, future_type, F > *ptr =
          new detail::future_continuation<BOOST_THREAD_FUTURE<R>, future_type, F>(*this, hexerboost::forward<F>(func), policy);
      if (ptr==0)
      {
  ...
  BOOST_THREAD_FUTURE<R>::then(BOOST_THREAD_RV_REF(F) func)
  {
    ...
    if (this->future_)
    {
      hexerboost::unique_lock<hexerboost::mutex> lock(this->future_->mutex);
      detail::future_continuation<BOOST_THREAD_FUTURE<R>, future_type, F > *ptr =
          new detail::future_continuation<BOOST_THREAD_FUTURE<R>, future_type, F>(*this, hexerboost::forward<F>(func));
      if (ptr==0)
      {
        return BOOST_THREAD_FUTURE<future_type>();
      }
