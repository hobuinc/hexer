...
#define BOOST_ONCE_INIT {0,0}
...
#ifdef BOOST_NO_ANSI_APIS
            return ::hexerboost::detail::win32::OpenEventW(
#else
            return ::hexerboost::detail::win32::OpenEventA(
#endif
                ::hexerboost::detail::win32::synchronize |
                ::hexerboost::detail::win32::event_modify_state,
                false,
                mutex_name);
        }
...
#ifdef BOOST_NO_ANSI_APIS
            return ::hexerboost::detail::win32::CreateEventW(
#else
            return ::hexerboost::detail::win32::CreateEventA(
#endif
                0,::hexerboost::detail::win32::manual_reset_event,
                ::hexerboost::detail::win32::event_initially_reset,
                mutex_name);
        }
    }
    ...
    void call_once(once_flag& flag,Function f)
    {
        ...
        while((status=::hexerboost::detail::interlocked_read_acquire(&flag.status))
              !=function_complete_flag_value)
        {
            ...
            if(!status)
            {
                BOOST_TRY
                {
                    ...
                    if(event_handle)
                    {
                        ::hexerboost::detail::win32::ResetEvent(event_handle);
                    }
                    f();
                    if(!counted)
                    {
                    ...
                    if(!event_handle &&
                       (::hexerboost::detail::interlocked_read_acquire(&flag.count)>1))
                    {
                        event_handle=detail::create_once_event(mutex_name,&flag);
                    }
                    if(event_handle)
                    {
                        ::hexerboost::detail::win32::SetEvent(event_handle);
                    }
                    break;
                }
                BOOST_CATCH(...)
            ...
            if(!counted)
            {
                BOOST_INTERLOCKED_INCREMENT(&flag.count);
                counted=true;
                status=::hexerboost::detail::interlocked_read_acquire(&flag.status);
            ...
            BOOST_VERIFY(!::hexerboost::detail::win32::WaitForSingleObject(
                             event_handle,::hexerboost::detail::win32::infinite));
        }
    }
}
