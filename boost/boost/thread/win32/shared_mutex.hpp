...
namespace hexerboost {} namespace boost = hexerboost; namespace hexerboost
{
    ...
    public:
        ...
        void lock_shared()
        {
            BOOST_VERIFY(timed_lock_shared(::hexerboost::detail::get_system_time_sentinel()));
        }
        ...
        bool timed_lock_shared(hexerboost::system_time const& wait_until)
        {
            for(;;)
            {
                state_data old_state=state;
                for(;;)
                {
                    ...
                    if(new_state.exclusive || new_state.exclusive_waiting_blocked)
                    {
                        ...
                        {
                            hexerboost::throw_exception(hexerboost::lock_error());
                        }
                    }
                    else
                    {
                ...
                unsigned long const res=detail::win32::WaitForSingleObject(semaphores[unlock_sem],::hexerboost::detail::get_milliseconds_until(wait_until));
        ...
        void lock()
        {
            BOOST_VERIFY(timed_lock(::hexerboost::detail::get_system_time_sentinel()));
        }
        ...
        bool timed_lock(hexerboost::system_time const& wait_until)
        {
            for(;;)
            {
                state_data old_state=state;
                ...
                #endif
                unsigned long const wait_res=detail::win32::WaitForMultipleObjects(2,semaphores,wait_all,::hexerboost::detail::get_milliseconds_until(wait_until));
