  ...
  namespace detail
  {
    ...
    public:
        ...
        explicit async_func(Fp&& f, Args&&... args)
            : f_(hexerboost::move(f), hexerboost::move(args)...) {}

        BOOST_SYMBOL_VISIBLE
        async_func(async_func&& f) : f_(hexerboost::move(f.f_)) {}
    ...
    private:
        ...
        {
            return invoke(hexerboost::move(std::get<0>(f_)), hexerboost::move(std::get<Indices>(f_))...);
        }
    };
#else
    template <class Fp>
    ...
    public:
        ...
        explicit async_func(BOOST_THREAD_FWD_REF(Fp) f)
            : f_(hexerboost::move(f)) {}

        BOOST_SYMBOL_VISIBLE
        async_func(BOOST_THREAD_FWD_REF(async_func) f) : f_(hexerboost::move(f.f_)) {}
