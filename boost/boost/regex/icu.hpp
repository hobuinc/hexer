...
class BOOST_REGEX_DECL icu_regex_traits_implementation
{
   ...
   typedef hexerboost::uint_least32_t        char_class_type;
public:
...
private:
   void init_error()
   {
      ...
      hexerboost::throw_exception(e);
   }
   U_NAMESPACE_QUALIFIER Locale m_locale;                                  // The ICU locale that we're using
   hexerboost::scoped_ptr< U_NAMESPACE_QUALIFIER Collator> m_collator;          // The full collation object
   hexerboost::scoped_ptr< U_NAMESPACE_QUALIFIER Collator> m_primary_collator;  // The primary collation object
};

inline hexerboost::shared_ptr<icu_regex_traits_implementation> get_icu_regex_traits_implementation(const U_NAMESPACE_QUALIFIER Locale& loc)
{
   return hexerboost::shared_ptr<icu_regex_traits_implementation>(new icu_regex_traits_implementation(loc));
}
...
public:
...
#else
   typedef hexerboost::uint64_t              char_class_type;
#endif
   ...
   ::hexerboost::regex_constants::syntax_type syntax_type(char_type c)const
   {
      return ((c < 0x7f) && (c > 0)) ? re_detail::get_default_syntax_type(static_cast<char>(c)) : regex_constants::syntax_char;
   }
   ::hexerboost::regex_constants::escape_syntax_type escape_syntax_type(char_type c) const
   {
   ...
   std::string error_string(::hexerboost::regex_constants::error_type n) const
   {
...
private:
   ...
   hexerboost::shared_ptr< ::hexerboost::re_detail::icu_regex_traits_implementation> m_pimpl;
};
...
inline u32regex do_make_u32regex(InputIterator i, 
                              InputIterator j, 
                              hexerboost::regex_constants::syntax_option_type opt, 
                              const hexerboost::mpl::int_<1>*)
{
   typedef hexerboost::u8_to_u32_iterator<InputIterator, UChar32> conv_type;
...
inline u32regex do_make_u32regex(InputIterator i, 
                              InputIterator j, 
                              hexerboost::regex_constants::syntax_option_type opt, 
                              const hexerboost::mpl::int_<2>*)
{
   typedef hexerboost::u16_to_u32_iterator<InputIterator, UChar32> conv_type;
   return u32regex(conv_type(i, i, j), conv_type(j, i, j), opt);
}
...
inline u32regex make_u32regex(InputIterator i, 
                              ...
                              hexerboost::regex_constants::syntax_option_type opt)
{
   return re_detail::do_make_u32regex(i, j, opt, static_cast<hexerboost::mpl::int_<sizeof(*i)> const*>(0));
}
//
// construction from UTF-8 nul-terminated strings:
//
inline u32regex make_u32regex(const char* p, hexerboost::regex_constants::syntax_option_type opt = hexerboost::regex_constants::perl)
{
   return re_detail::do_make_u32regex(p, p + std::strlen(p), opt, static_cast<hexerboost::mpl::int_<1> const*>(0));
}
inline u32regex make_u32regex(const unsigned char* p, hexerboost::regex_constants::syntax_option_type opt = hexerboost::regex_constants::perl)
{
   return re_detail::do_make_u32regex(p, p + std::strlen(reinterpret_cast<const char*>(p)), opt, static_cast<hexerboost::mpl::int_<1> const*>(0));
}
//
// construction from UTF-16 nul-terminated strings:
//
#ifndef BOOST_NO_WREGEX
inline u32regex make_u32regex(const wchar_t* p, hexerboost::regex_constants::syntax_option_type opt = hexerboost::regex_constants::perl)
{
   return re_detail::do_make_u32regex(p, p + std::wcslen(p), opt, static_cast<hexerboost::mpl::int_<sizeof(wchar_t)> const*>(0));
}
#endif
#if !defined(U_WCHAR_IS_UTF16) && (U_SIZEOF_WCHAR_T != 2)
inline u32regex make_u32regex(const UChar* p, hexerboost::regex_constants::syntax_option_type opt = hexerboost::regex_constants::perl)
{
   return re_detail::do_make_u32regex(p, p + u_strlen(p), opt, static_cast<hexerboost::mpl::int_<2> const*>(0));
}
#endif
//
...
inline u32regex make_u32regex(const std::basic_string<C, T, A>& s, hexerboost::regex_constants::syntax_option_type opt = hexerboost::regex_constants::perl)
{
   return re_detail::do_make_u32regex(s.begin(), s.end(), opt, static_cast<hexerboost::mpl::int_<sizeof(C)> const*>(0));
}
//
// Construction from ICU string type:
//
inline u32regex make_u32regex(const U_NAMESPACE_QUALIFIER UnicodeString& s, hexerboost::regex_constants::syntax_option_type opt = hexerboost::regex_constants::perl)
{
   return re_detail::do_make_u32regex(s.getBuffer(), s.getBuffer() + s.length(), opt, static_cast<hexerboost::mpl::int_<2> const*>(0));
}
...
inline bool do_regex_match(BidiIterator first, BidiIterator last, 
                 ...
                 hexerboost::mpl::int_<4> const*)
{
   return ::hexerboost::regex_match(first, last, m, e, flags);
}
template <class BidiIterator, class Allocator>
bool do_regex_match(BidiIterator first, BidiIterator last, 
                 ...
                 const u32regex& e, 
                 match_flag_type flags,
                 hexerboost::mpl::int_<2> const*)
{
   typedef u16_to_u32_iterator<BidiIterator, UChar32> conv_type;
   typedef match_results<conv_type>                   match_type;
   typedef typename match_type::allocator_type        alloc_type;
   match_type what;
   bool result = ::hexerboost::regex_match(conv_type(first, first, last), conv_type(last, first, last), what, e, flags);
...
inline bool do_regex_search(BidiIterator first, BidiIterator last, 
                 ...
                 hexerboost::mpl::int_<4> const*)
{
   return ::hexerboost::regex_search(first, last, m, e, flags, base);
}
template <class BidiIterator, class Allocator>
                 ...
                 hexerboost::mpl::int_<2> const*)
{
   ...
   typedef typename match_type::allocator_type        alloc_type;
   match_type what;
   bool result = ::hexerboost::regex_search(conv_type(first, first, last), conv_type(last, first, last), what, e, flags, conv_type(base));
   // copy results across to m:
   if(result) copy_results(m, what);
   return result;
}
...
namespace re_detail{
template <class I>
inline std::pair< hexerboost::u8_to_u32_iterator<I>, hexerboost::u8_to_u32_iterator<I> >
   make_utf32_seq(I i, I j, mpl::int_<1> const*)
{
   return std::pair< hexerboost::u8_to_u32_iterator<I>, hexerboost::u8_to_u32_iterator<I> >(hexerboost::u8_to_u32_iterator<I>(i, i, j), hexerboost::u8_to_u32_iterator<I>(j, i, j));
}
template <class I>
inline std::pair< hexerboost::u16_to_u32_iterator<I>, hexerboost::u16_to_u32_iterator<I> >
   make_utf32_seq(I i, I j, mpl::int_<2> const*)
{
   return std::pair< hexerboost::u16_to_u32_iterator<I>, hexerboost::u16_to_u32_iterator<I> >(hexerboost::u16_to_u32_iterator<I>(i, i, j), hexerboost::u16_to_u32_iterator<I>(j, i, j));
}
template <class I>
inline std::pair< I, I >
...
inline std::pair< hexerboost::u8_to_u32_iterator<const charT*>, hexerboost::u8_to_u32_iterator<const charT*> >
   make_utf32_seq(const charT* p, mpl::int_<1> const*)
{
   std::size_t len = std::strlen((const char*)p);
   return std::pair< hexerboost::u8_to_u32_iterator<const charT*>, hexerboost::u8_to_u32_iterator<const charT*> >(hexerboost::u8_to_u32_iterator<const charT*>(p, p, p+len), hexerboost::u8_to_u32_iterator<const charT*>(p+len, p, p+len));
}
template <class charT>
inline std::pair< hexerboost::u16_to_u32_iterator<const charT*>, hexerboost::u16_to_u32_iterator<const charT*> >
   make_utf32_seq(const charT* p, mpl::int_<2> const*)
{
   std::size_t len = u_strlen((const UChar*)p);
   return std::pair< hexerboost::u16_to_u32_iterator<const charT*>, hexerboost::u16_to_u32_iterator<const charT*> >(hexerboost::u16_to_u32_iterator<const charT*>(p, p, p + len), hexerboost::u16_to_u32_iterator<const charT*>(p+len, p, p + len));
}
template <class charT>
                                 ...
                                 )
{
   ...
   else
   {
      ...
      while(i != j)
      {
         ...
         if(f.size())
            out = ::hexerboost::re_detail::regex_format_imp(out, *i, &*f.begin(), &*f.begin() + f.size(), flags, e.get_traits());
         else
            out = ::hexerboost::re_detail::regex_format_imp(out, *i, static_cast<UChar32 const*>(0), static_cast<UChar32 const*>(0), flags, e.get_traits());
         last_m = (*i)[0].second;
