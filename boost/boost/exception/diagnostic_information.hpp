...
hexerboost
    ...
    exception_detail
        {
        std::string diagnostic_information_impl( hexerboost::exception const *, std::exception const *, bool );
        }
    ...
    current_exception_diagnostic_information()
        {
        hexerboost::exception const * be=current_exception_cast<hexerboost::exception const>();
...
hexerboost
    ...
    exception_detail
        ...
        inline
        std::string
        diagnostic_information_impl( hexerboost::exception const * be, std::exception const * se, bool with_what )
            {
            if( !be && !se )
                return "Unknown exception.";
#ifndef BOOST_NO_RTTI
            if( !be )
                be=dynamic_cast<hexerboost::exception const *>(se);
            if( !se )
    ...
    diagnostic_information_what( exception const & e ) throw()
        ...
        try
            ...
            if( char const * di=exception_detail::get_diagnostic_information(e,0) )
                ...
                return "Failed to produce hexerboost::diagnostic_information_what()";
#ifndef BOOST_NO_EXCEPTIONS
            }
        catch(
        ... )
