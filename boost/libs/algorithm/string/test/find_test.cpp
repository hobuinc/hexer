...
void find_test()
{
    ...
    std::vector<hexerboost::iterator_range<std::string::iterator> > overlap_results;
    hexerboost::algorithm::find_all(overlap_results, overlap_target, string("aaa"));
    BOOST_CHECK( overlap_results.size() == 2 );

    std::string overlap_target2("aaaabbbbaaaa");
    hexerboost::algorithm::find_all(overlap_results, overlap_target2, string("bb"));
    BOOST_CHECK( overlap_results.size() == 3 );
    hexerboost::algorithm::find_all(overlap_results, overlap_target2, string("aa"));
