...
{
  using namespace hexerboost;

  typedef typename std::iterator_traits<CIterator>::value_type Value;
  typedef hexerboost::reverse_iterator<CIterator> RCIterator;
  ...
  tie( hexerboost::minmax_element(first, last), min, max );
  ...
  lc.reset();
  tie( hexerboost::minmax_element(first, last, lc), min, max );
  ...
  // boost extensions
  // first version, operator<
  CHECK_EQUAL_ITERATORS( hexerboost::first_min_element(first, last), std::min_element(first, last), first );
  rmin = RCIterator(hexerboost::last_min_element(first, last));
  rmin = (rmin == rfirst) ? rlast : --rmin;
  CHECK_EQUAL_ITERATORS( rmin, std::min_element(rfirst, rlast), rfirst );
  CHECK_EQUAL_ITERATORS( hexerboost::first_max_element(first, last), std::max_element(first, last), first );
  rmax = RCIterator(hexerboost::last_max_element(first, last));
  rmax = (rmax == rfirst) ? rlast : --rmax;
  CHECK_EQUAL_ITERATORS( rmax, std::max_element(rfirst, rlast), rfirst );
  tie( hexerboost::first_min_last_max_element(first, last), min, max );
  CHECK_EQUAL_ITERATORS( min, hexerboost::first_min_element(first, last), first );
  CHECK_EQUAL_ITERATORS( max, hexerboost::last_max_element(first, last), first );
  tie( hexerboost::last_min_first_max_element(first, last), min, max );
  CHECK_EQUAL_ITERATORS( min, hexerboost::last_min_element(first, last), first );
  CHECK_EQUAL_ITERATORS( max, hexerboost::first_max_element(first, last), first );
  tie( hexerboost::last_min_last_max_element(first, last), min, max );
  CHECK_EQUAL_ITERATORS( min, hexerboost::last_min_element(first, last), first );
  CHECK_EQUAL_ITERATORS( max, hexerboost::last_max_element(first, last), first );

  // second version, comp function object (keeps a counter!)
  lc.reset();
  min = hexerboost::first_min_element(first, last, lc);
  BOOST_CHECK( counter <= opt_min_count(n) );
  CHECK_EQUAL_ITERATORS( min, std::min_element(first, last, lc), first );
  lc.reset();
  rmin = RCIterator(hexerboost::last_min_element(first, last, lc));
  rmin = (rmin == rfirst) ? rlast : --rmin;
  BOOST_CHECK( counter <= opt_min_count(n) );
  CHECK_EQUAL_ITERATORS( rmin, std::min_element(rfirst, rlast, lc), rfirst );
  lc.reset();
  max =  hexerboost::first_max_element(first, last, lc);
  BOOST_CHECK( counter <= opt_min_count(n) );
  CHECK_EQUAL_ITERATORS( max, std::max_element(first, last, lc), first );
  lc.reset();
  rmax = RCIterator(hexerboost::last_max_element(first, last, lc));
  rmax = (rmax == rfirst) ? rlast : --rmax;
  BOOST_CHECK( counter <= opt_min_count(n) );
  CHECK_EQUAL_ITERATORS( rmax, std::max_element(rfirst, rlast, lc), rfirst );
  lc.reset();
  tie( hexerboost::first_min_last_max_element(first, last, lc), min, max );
  BOOST_CHECK( counter <= opt_boost_minmax_count(n) );
  CHECK_EQUAL_ITERATORS( min, hexerboost::first_min_element(first, last, lc), first );
  CHECK_EQUAL_ITERATORS( max, hexerboost::last_max_element(first, last, lc), first );
  lc.reset();
  tie( hexerboost::last_min_first_max_element(first, last, lc), min, max );
  BOOST_CHECK( counter <= opt_boost_minmax_count(n) );
  BOOST_CHECK( min == hexerboost::last_min_element(first, last, lc) );
  CHECK_EQUAL_ITERATORS( max, hexerboost::first_max_element(first, last, lc), first );
  lc.reset();
  tie( hexerboost::last_min_last_max_element(first, last, lc), min, max );
  BOOST_CHECK( counter <= opt_minmax_count(n) );
  CHECK_EQUAL_ITERATORS( min, hexerboost::last_min_element(first, last, lc), first );
  CHECK_EQUAL_ITERATORS( max, hexerboost::last_max_element(first, last, lc), first );
}
