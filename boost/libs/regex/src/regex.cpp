 ...
 */
  ...
  *   DESCRIPTION: Misc hexerboost::regbase member funnctions.
  */
...
regex_error::regex_error(regex_constants::error_type err) 
   : std::runtime_error(::hexerboost::re_detail::get_default_error_string(err))
   , m_error_code(err)
   , m_position(0) 
{
}
...
void regex_error::raise()const
{
   ...
   ::hexerboost::throw_exception(*this);
#endif
}
...
{
   ::hexerboost::throw_exception(ex);
}
//
// error checking API:
//
BOOST_REGEX_DECL void BOOST_REGEX_CALL verify_options(hexerboost::regex::flag_type /*ef*/, match_flag_type mf)
{
