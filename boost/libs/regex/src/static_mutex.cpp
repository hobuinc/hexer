...
BOOST_STATIC_ASSERT(sizeof(LONG) == sizeof(hexerboost::int32_t));
...
void scoped_static_mutex_lock::lock()
{
   if(0 == m_have_lock)
   {
      ...
      while(0 != InterlockedCompareExchange(reinterpret_cast<void**>((hexerboost::uint_least16_t*)&(m_mutex.m_mutex)), (void*)1, 0))
#else
...
hexerboost::recursive_mutex* static_mutex::m_pmutex = 0;
hexerboost::once_flag static_mutex::m_once = BOOST_ONCE_INIT;

extern "C" BOOST_REGEX_DECL void hexerboost_regex_free_static_mutex()
{
...
{
   m_pmutex = new hexerboost::recursive_mutex();
   int r = atexit(hexerboost_regex_free_static_mutex);
   BOOST_ASSERT(0 == r);
}
...
void scoped_static_mutex_lock::lock()
{
   if(0 == m_have_lock)
   {
       hexerboost::call_once(static_mutex::m_once,&static_mutex::init);
      if(0 == m_plock)
         m_plock = new hexerboost::recursive_mutex::scoped_lock(*static_mutex::m_pmutex, hexerboost::defer_lock);
      m_plock->lock();
      m_have_lock = true;
   }
}
