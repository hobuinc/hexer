...
#define TLS_OUT_OF_INDEXES 0xFFFFFFFF
        ...
        {
            hexerboosttss_cleanup_implemented(); // if anyone uses TSS, we need the cleanup linked in
            ...
            if(current_thread_data)
            {
                while(! current_thread_data->tss_data.empty() || current_thread_data->thread_exit_callbacks)
                {
                    while(current_thread_data->thread_exit_callbacks)
                    {
                        ...
                        {
                            (*current_node->func)();
                            hexerboost::detail::heap_delete(current_node->func);
                        }
                        hexerboost::detail::heap_delete(current_node);
                    }
...
//          hexerboost::throw_exception(thread_resource_error());
...
#if defined BOOST_THREAD_USES_DATETIME
    bool thread::timed_join(hexerboost::system_time const& wait_until)
    {
    ...
    BOOST_THREAD_DECL void __cdecl hexerbooston_process_enter()
    {}

    BOOST_THREAD_DECL void __cdecl hexerbooston_thread_enter()
    {}

    BOOST_THREAD_DECL void __cdecl hexerbooston_process_exit()
    {
        hexerboost::cleanup_tls_key();
    }

    BOOST_THREAD_DECL void __cdecl hexerbooston_thread_exit()
    {
        hexerboost::run_thread_exit_callbacks();
    }
