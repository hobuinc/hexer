...
bool test_allocation_shrink(Allocator &a)
{
   ...
   for(int i = 0, max = (int)buffers.size()
      ...
      if(a.template allocation_command<char>
         ( hexerboost::interprocess::shrink_in_place | hexerboost::interprocess::nothrow_allocation, i*2
...
bool test_allocation_expand(Allocator &a)
{
   ...
   for(int i = 0, max = (int)buffers.size()
      ...
      while(a.template allocation_command<char>
         ( hexerboost::interprocess::expand_fwd | hexerboost::interprocess::nothrow_allocation, min_size
...
bool test_allocation_shrink_and_expand(Allocator &a)
{
   ...
   for(int i = 0; true; ++i){
      ...
      void *ptr = a.template allocation_command<char>
         ( hexerboost::interprocess::allocate_new | hexerboost::interprocess::nothrow_allocation, i, i*2, received_size).first;
      if(!ptr){
         ptr = a.template allocation_command<char>
            ( hexerboost::interprocess::allocate_new | hexerboost::interprocess::nothrow_allocation, 1, i*2, received_size).first;
         if(!ptr)
            break;
      }
      buffers.push_back(ptr);
   ...
   for(int i = 0, max = (int)buffers.size()
      ...
      typename Allocator::size_type received_size;
      if(a.template allocation_command<char>
         ( hexerboost::interprocess::shrink_in_place | hexerboost::interprocess::nothrow_allocation, received_sizes[i]
         , i, received_size, static_cast<char*>(buffers[i])).first){
   ...
   for(int i = 0, max = (int)buffers.size()
      ...
      if(a.template allocation_command<char>
         ( hexerboost::interprocess::expand_fwd | hexerboost::interprocess::nothrow_allocation, request_size
   ...
   for(int sizeof_object = 1; sizeof_object < 20; ++sizeof_object){
      ...
      for(int i = 0; true; ++i){
         ...
         std::pair<void*, bool> ret = a.raw_allocation_command
            ( hexerboost::interprocess::expand_bwd | hexerboost::interprocess::nothrow_allocation, min_size
...
bool test_many_deallocation(Allocator &a)
{
...

   {
      for(int i = 0; true; ++i){
         ...
         if(chain.empty())
            break;
         buffers.push_back(hexerboost::move(chain));
      }
