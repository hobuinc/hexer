...
hexerboost::posix_time::ptime ptime_delay(int secs)
{
   return   microsec_clock::universal_time() +
            hexerboost::posix_time::time_duration(0, 0, secs);
}

inline hexerboost::xtime delay(int secs, int msecs=0, int nsecs=0)
{
    ...
    int ret = hexerboost::xtime_get(&xt, hexerboost::TIME_UTC_);
    BOOST_INTERPROCES_CHECK(ret == static_cast<int>(hexerboost::TIME_UTC_));(void)ret;
    #else
    int ret = hexerboost::xtime_get(&xt, hexerboost::TIME_UTC);
    BOOST_INTERPROCES_CHECK(ret == static_cast<int>(hexerboost::TIME_UTC));(void)ret;
    #endif
    nsecs += xt.nsec;
...
{
    hexerboost::interprocess::scoped_lock<Mutex>
      lock(data->mutex);
...
void do_test_condition_notify_one()
{
   ...
   hexerboost::thread thread(bind_function(&condition_test_thread<Condition, Mutex>, &data));
   //Make sure thread is blocked
   hexerboost::thread::sleep(delay(1));
   {
      hexerboost::interprocess::scoped_lock<Mutex>
         lock(data.mutex);
      BOOST_INTERPROCES_CHECK(lock ? true : false);
...
void do_test_condition_notify_all()
{
   ...
   hexerboost::thread_group threads;
   condition_test_data<Condition, Mutex> data;
...
void do_test_condition_waits()
{
   ...
   hexerboost::thread thread(bind_function(&condition_test_waits<Condition, Mutex>, &data));
...
struct condition_func
{
   ...
   void operator()()
   {
      hexerboost::interprocess::scoped_lock<Mutex>lock(mutex_);
      while(count == 0){
         ++waiting_readers;
...
void do_test_condition_queue_notify_one(void)
{
   ...
   //Create threads that will decrease count
   {
      ...
      hexerboost::thread_group thgroup;
      int i;
      ...
      for(i = 0; i < NumThreads; ++i){
         hexerboost::interprocess::scoped_lock<Mutex> lock(mutex);
