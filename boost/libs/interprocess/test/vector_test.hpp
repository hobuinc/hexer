...
bool copyable_only(V1 *, V2 *, hexerboost::interprocess::ipcdetail::false_type)
{
   return true;
}
...
template<class V1, class V2>
bool copyable_only(V1 *shmvector, V2 *stdvector, hexerboost::interprocess::ipcdetail::true_type)
{
...

   {
      ...
      shmvector->insert(shmvector->begin()+size/2, 50, hexerboost::move(move_me));
      if(!test::CheckEqualContainers(shmvector, stdvector)) return false;
   }
   {
      IntType move_me(2);
      shmvector->assign(shmvector->size()/2, hexerboost::move(move_me));
      stdvector->assign(stdvector->size()/2, 2);
      if(!test::CheckEqualContainers(shmvector, stdvector)) return false;
   }
   {
      IntType move_me(3);
      shmvector->assign(shmvector->size()*3-1, hexerboost::move(move_me));
...
int vector_test()
{
...

   {
      ...
      try{
         ...
         for(int i = 0; i < max; ++i){
            IntType new_int(i);
            shmvector->insert(shmvector->end(), hexerboost::move(new_int));
...

         {
            ...
            for(int i = 0; i < 50; ++i){
               IntType new_int(-1);
               //BOOST_STATIC_ASSERT((::hexerboost::move_ipcdetail::is_copy_constructible<hexerboost::interprocess::test::movable_int>::value == false));
               aux_vect[i] = hexerboost::move(new_int);
            }
            int aux_vect2[50];
            for(int i = 0; i < 50; ++i){
               aux_vect2[i] = -1;
         ...
         shmvector->push_back(hexerboost::move(push_back_this));
         ...
         for(int i = 0; i < max; ++i){
            ...
            shmvector->insert(shmvector->begin(), hexerboost::move(insert_this));
