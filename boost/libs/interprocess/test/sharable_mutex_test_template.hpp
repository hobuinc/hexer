...
void plain_exclusive(void *arg, SM &sm)
{
   ...
   hexerboost::interprocess::scoped_lock<SM> l(sm);
   hexerboost::thread::sleep(xsecs(3*BaseSeconds));
   shared_val += 10;
...
{
   data<SM> *pdata = static_cast<data<SM>*>(arg);
   hexerboost::interprocess::sharable_lock<SM> l(sm);
   if(pdata->m_secs){
      hexerboost::thread::sleep(xsecs(pdata->m_secs*BaseSeconds));
   }
   pdata->m_value = shared_val;
}
...
{
   data<SM> *pdata = static_cast<data<SM>*>(arg);
   hexerboost::interprocess::scoped_lock<SM> l(sm, hexerboost::interprocess::defer_lock);
   if (l.try_lock()){
      hexerboost::thread::sleep(xsecs(3*BaseSeconds));
      shared_val += 10;
      pdata->m_value = shared_val;
...
{
   data<SM> *pdata = static_cast<data<SM>*>(arg);
   hexerboost::interprocess::sharable_lock<SM> l(sm, hexerboost::interprocess::defer_lock);
   if (l.try_lock()){
      if(pdata->m_secs){
         hexerboost::thread::sleep(xsecs(pdata->m_secs*BaseSeconds));
      }
...
void timed_exclusive(void *arg, SM &sm)
{
   ...
   hexerboost::interprocess::scoped_lock<SM>
...
void timed_shared(void *arg, SM &sm)
{
   data<SM> *pdata = static_cast<data<SM>*>(arg);
   hexerboost::posix_time::ptime pt(delay(pdata->m_secs));
   hexerboost::interprocess::sharable_lock<SM>
      l(sm, hexerboost::interprocess::defer_lock);
   if (l.timed_lock(pt)){
      if(pdata->m_secs){
         hexerboost::thread::sleep(xsecs(pdata->m_secs*BaseSeconds));
...
{
   {
      ...
      hexerboost::thread tw1(thread_adapter<SM>(plain_exclusive, &e1, mtx));
      ...
      hexerboost::thread tw2(thread_adapter<SM>(plain_exclusive, &e2, mtx));

      // Reader one launches, "clearly" after writer two, and "clearly"
      //   while writer 1 still holds the lock
      hexerboost::thread::sleep(xsecs(1*BaseSeconds));
      hexerboost::thread thr1(thread_adapter<SM>(plain_shared,&s1, mtx));
      hexerboost::thread thr2(thread_adapter<SM>(plain_shared,&s2, mtx));
...
void test_try_sharable_mutex()
{
   ...
   hexerboost::thread tw1(thread_adapter<SM>(try_exclusive,&e1,mtx));

   // Reader one launches, "clearly" after writer #1 holds the lock
   //   and before it releases the lock.
   hexerboost::thread::sleep(xsecs(1*BaseSeconds));
   hexerboost::thread thr1(thread_adapter<SM>(try_shared,&s1,mtx));

   // Writer two launches in the same timeframe.
   hexerboost::thread tw2(thread_adapter<SM>(try_exclusive,&e2,mtx));
...
void test_timed_sharable_mutex()
{
   ...
   hexerboost::thread tw1(thread_adapter<SM>(timed_exclusive,&e1,mtx));
   ...
   hexerboost::thread tw2(thread_adapter<SM>(timed_exclusive,&e2,mtx));
   ...
   hexerboost::thread thr1(thread_adapter<SM>(timed_shared,&s1,mtx));
   hexerboost::thread thr2(thread_adapter<SM>(timed_shared,&s2,mtx));
