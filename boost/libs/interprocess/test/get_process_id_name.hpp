...
inline void get_process_id_name(std::string &str)
{
   ...
   sstr << "process_" << hexerboost::interprocess::ipcdetail::get_current_process_id() << std::ends;
   str = sstr.str().c_str();
}
...
{
   std::stringstream sstr;
   sstr << "process_" << hexerboost::interprocess::ipcdetail::get_current_process_id() << "_" << ptr << std::ends;
   str = sstr.str().c_str();
}
