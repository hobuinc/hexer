...
int main()
{
   ...
   shared_memory_object::remove(process_name.c_str());
   {
      ...
      my_unique_ptr_class my_ptr4(hexerboost::move(my_ptr3));

      //Construct a list and fill
      MyList list(segment.get_segment_manager());

      //Insert from my_unique_ptr_class
      list.push_front(hexerboost::move(my_ptr));
      list.push_back(hexerboost::move(my_ptr2));
      ...
      set.insert(hexerboost::move(*list.begin()));
      set.insert(hexerboost::move(*list.rbegin()));
      ...
      //Insert from my_unique_ptr_class
      if(ptr1 < ptr2){
         vector.insert(vector.begin(), hexerboost::move(*set.begin()));
         vector.insert(vector.end(),   hexerboost::move(*set.rbegin()));
      }
      else{
         vector.insert(vector.begin(), hexerboost::move(*set.rbegin()));
         vector.insert(vector.end(),   hexerboost::move(*set.begin()));
      }
      ...
      MyVector vector2(hexerboost::move(vector));
      vector2.swap(vector);
