...
inline void sleep(const hexerboost::posix_time::ptime &xt)
{
   hexerboost::interprocess::ipcdetail::thread_sleep
      ((xt - microsec_clock::universal_time()).total_milliseconds());
}

inline hexerboost::posix_time::ptime delay(int secs, int msecs=0, int nsecs = 0)
{
   (void)msecs;
   using namespace hexerboost::posix_time;
   ...
   hexerboost::posix_time::ptime cur = microsec_clock::universal_time();
   return cur +=  hexerboost::posix_time::time_duration(0, 0, secs, count);
}

inline bool in_range(const hexerboost::posix_time::ptime& xt, int secs=1)
{
    hexerboost::posix_time::ptime min = delay(-secs);
    hexerboost::posix_time::ptime max = delay(0);
    return (xt > min) && (max > xt);
}

hexerboost::xtime xsecs(int secs)
{
   hexerboost::xtime ret;
   #if BOOST_VERSION >= 105100 //TIME_UTC is a macro in C11, breaking change in Boost.Thread
   hexerboost::xtime_get(&ret, hexerboost::TIME_UTC_);
   #else
   hexerboost::xtime_get(&ret, hexerboost::TIME_UTC);
   #endif
   ret.sec += secs;
   return ret;
}
