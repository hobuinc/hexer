...
static hexerboost::interprocess::message_queue *pmessage_queue;

void receiver()
{
   hexerboost::interprocess::message_queue::size_type recvd_size;
   unsigned int priority;
   int nummsg = NumMsg;
...
{
   hexerboost::interprocess::message_queue::remove(test::get_process_id_name());
   {
      std::auto_ptr<hexerboost::interprocess::message_queue>
         ptr(new hexerboost::interprocess::message_queue
               (create_only, test::get_process_id_name(), 10, 10));
      pmessage_queue = ptr.get();

      //Launch the receiver thread
      hexerboost::thread thread(&receiver);
      hexerboost::thread::yield();
