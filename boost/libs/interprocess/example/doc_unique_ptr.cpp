...
int main ()
{
   ...
   //<-
   #if 1
   std::string mapped_file(hexerboost::interprocess::ipcdetail::get_temporary_path());
   ...
   } remover(MappedFile);
   {
      ...
      for(int i = 0; i < 100; ++i){
         ...
         unique_vector->push_back(hexerboost::move(p));
      ...
      for(int i = 99; !unique_vector->empty(); --i){
         unique_list->push_front(hexerboost::move(unique_vector->back()));
