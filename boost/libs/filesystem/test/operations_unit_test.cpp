...
using namespace hexerboost::filesystem;
using namespace hexerboost::system;
using std::cout;
using std::endl;
using std::string;
...
namespace
{
  ...
  void query_test()
  {
    ...
    CHECK(file_size("no-such-file", ec) == static_cast<hexerboost::uintmax_t>(-1));
    ...
    CHECK(!hexerboost::filesystem::is_empty("/"));
    CHECK(!is_other("/"));
  }
...
//      cout << "  " << it->path().string() << "\n";
  ...
  void operations_test()
  {
    ...
    CHECK(!hexerboost::filesystem::remove("no-such-file-or-directory"));
  ...
  void error_handling_test()
  {
    ...
    catch (const hexerboost::filesystem::filesystem_error & ex)
    {
      threw = true;
