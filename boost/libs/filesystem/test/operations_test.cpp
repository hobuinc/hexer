...
using hexerboost::system::system_error;
...
namespace
{
  ...
  hexerboost::system::error_category* poison_category_aux() { return 0; }
  hexerboost::system::error_category& poison_category()     { return *poison_category_aux(); }
    ...
    catch (std::runtime_error x)
    {
      ...
      if (platform == "Windows" && language_id == 0x0409) // English (United States)
        ...
        BOOST_TEST(std::strncmp(x.what(),
          "hexerboost::filesystem::create_directory",
          sizeof("hexerboost::filesystem::create_directory")-1) == 0);
    }
    ...
    catch (system_error x)
    {
      ...
      if (platform == "Windows" && language_id == 0x0409) // English (United States)
        BOOST_TEST(std::strcmp(x.what(),
          "hexerboost::filesystem::create_directory: The system cannot find the path specified") == 0);
    }
    BOOST_TEST(exception_thrown);
    ...
    catch (fs::filesystem_error x)
    {
      ...
      if (platform == "Windows" && language_id == 0x0409) // English (United States)
      {
        bool ok (std::strcmp(x.what(),
          "hexerboost::filesystem::create_directory: The system cannot find the path specified: \"no-such-dir/foo/bar\"") == 0);
        BOOST_TEST(ok);
        if (!ok)
        {
  ...
  void directory_iterator_tests()
  {
    ...
    try
    {
      ...
      BOOST_TEST(ec);
      BOOST_TEST(ec == hexerboost::system::errc::no_such_file_or_directory);
    }
    catch (const fs::filesystem_error &) { dir_itr_exception = true; }
    BOOST_TEST(!dir_itr_exception);
...
//      BOOST_TEST(fs::exists((*it).path()));
  ...
  void recursive_directory_iterator_tests()
  {
    ...
    hexerboost::system::error_code ec;
    int d1f1_count = 0;
...
# endif
  ...
  cout << "sizeof(hexerboost::uintmax_t) = " << sizeof(hexerboost::uintmax_t) << '\n';
  BOOST_TEST(sizeof(hexerboost::uintmax_t) >= 8);
