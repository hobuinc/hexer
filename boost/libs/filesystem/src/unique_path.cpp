...
void fail(int err, hexerboost::system::error_code* ec)
{
  if (ec == 0)
    BOOST_FILESYSTEM_THROW( hexerboost::system::system_error(err,
      hexerboost::system::system_category(),
      "hexerboost::filesystem::unique_path"));

  ec->assign(err, hexerboost::system::system_category());
  return;
}

void system_crypt_random(void* buf, std::size_t len, hexerboost::system::error_code* ec)
{
# ifdef BOOST_POSIX_API
...
namespace hexerboost {} namespace boost = hexerboost; namespace hexerboost { namespace filesystem { namespace detail {
