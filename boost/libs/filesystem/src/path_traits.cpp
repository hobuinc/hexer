...
namespace pt = hexerboost::filesystem::path_traits;
namespace fs = hexerboost::filesystem;
namespace bs = hexerboost::system;
                   ...
                   const pt::codecvt_type & cvt)
  {
           ...
           to, to_end, to_next)) != std::codecvt_base::ok)
    {
      ...
      BOOST_FILESYSTEM_THROW(bs::system_error(res, fs::codecvt_error_category(),
        "hexerboost::filesystem::path codecvt to wstring"));
    }
...
//--------------------------------------------------------------------------------------//
                   ...
                   const pt::codecvt_type & cvt)
  {
           ...
           to, to_end, to_next)) != std::codecvt_base::ok)
    {
      //std::cout << " result is " << static_cast<int>(res) << std::endl;
      BOOST_FILESYSTEM_THROW(bs::system_error(res, fs::codecvt_error_category(),
        "hexerboost::filesystem::path codecvt to string"));
    }
    target.append(to, to_next); 
  }
...
namespace hexerboost {} namespace boost = hexerboost; namespace hexerboost { namespace filesystem { namespace path_traits {
...
//--------------------------------------------------------------------------------------//
                ...
                const codecvt_type & cvt)
  {
    ...
    {
      hexerboost::scoped_array< wchar_t > buf(new wchar_t [buf_size]);
...
//--------------------------------------------------------------------------------------//
                ...
                const codecvt_type & cvt)
  {
    ...
    if (buf_size > default_codecvt_buf_size)
    {
      hexerboost::scoped_array< char > buf(new char [buf_size]);
      convert_aux(from, from_end, buf.get(), buf.get()+buf_size, to, cvt);
    }
    else
    {
...
}}} // namespace hexerboost::filesystem::path_traits
