...
namespace fs = hexerboost::filesystem;
using hexerboost::filesystem::path;
using hexerboost::filesystem::filesystem_error;
using hexerboost::filesystem::perms;
using hexerboost::system::error_code;
using hexerboost::system::error_category;
using hexerboost::system::system_category;
using std::string;
using std::wstring;
...
#     define BOOST_STATVFS_F_FRSIZE static_cast<hexerboost::uintmax_t>(vfs.f_bsize)
#   endif
#   include <dirent.h>
#   include <unistd.h>
#   include <fcntl.h>
...
namespace
{
  ...
  hexerboost::filesystem::directory_iterator end_dir_itr;
...
#     endif
    ...
    {
      if (error(!remove_directory(p), p, ec, "hexerboost::filesystem::remove"))
        return false;
    }
    else
    {
      if (error(!remove_file(p), p, ec, "hexerboost::filesystem::remove"))
        return false;
    }
    return true;
  }

  hexerboost::uintmax_t remove_all_aux(const path& p, fs::file_type type,
    error_code* ec)
  {
    hexerboost::uintmax_t count = 1;
...
//--------------------------------------------------------------------------------------//
    ...
    const std::string& to_p, bool fail_if_exists)
  {
    ...
    hexerboost::scoped_array<char> buf(new char [buf_sz]);
...
#   endif
  ...
  bool is_reparse_point_a_symlink(const path& p)
  {
    ...
    hexerboost::scoped_array<char> buf(new char [MAXIMUM_REPARSE_DATA_BUFFER_SIZE]);    
  ...
  fs::file_status process_status_failure(const path& p, error_code* ec)
  {
    ...
    if (ec == 0)
      BOOST_FILESYSTEM_THROW(filesystem_error("hexerboost::filesystem::status",
  ...
  BOOL resize_file_api(const wchar_t* p, hexerboost::uintmax_t size)
  {
...
#   endif
  ...
  path canonical(const path& p, const path& base, system::error_code* ec)
  {
    ...
    if (stat.type() == fs::file_not_found)
    {
      if (ec == 0)
        BOOST_FILESYSTEM_THROW(filesystem_error(
          "hexerboost::filesystem::canonical", source,
    ...
    else if (local_ec)
    {
      if (ec == 0)
        BOOST_FILESYSTEM_THROW(filesystem_error(
          "hexerboost::filesystem::canonical", source, local_ec));
      *ec = local_ec;
      return result;
    }
  ...
  void copy(const path& from, const path& to, system::error_code* ec)
  {
    ...
    {
      if (ec == 0)
        BOOST_FILESYSTEM_THROW(filesystem_error("hexerboost::filesystem::copy",
...
#   endif
    error(!BOOST_COPY_DIRECTORY(from.c_str(), to.c_str()),
      from, to, ec, "hexerboost::filesystem::copy_directory");
  }
                  ...
                  error_code* ec)
  {
    error(!BOOST_COPY_FILE(from.c_str(), to.c_str(),
      option == copy_option::fail_if_exists),
        from, to, ec, "hexerboost::filesystem::copy_file");
  }
...
# if defined(_WIN32_WINNT) && _WIN32_WINNT < 0x0600
    error(true, error_code(BOOST_ERROR_NOT_SUPPORTED, system_category()),
      ...
      "hexerboost::filesystem::copy_symlink");
...
# endif
 ...
 BOOST_FILESYSTEM_DECL
    ...
    if (!parent.empty())
    {
      ...
      if (parent_status.type() == file_not_found)
      {
        ...
        if (local_ec)
        {
          if (ec == 0)
            BOOST_FILESYSTEM_THROW(filesystem_error(
              "hexerboost::filesystem::create_directories", parent, local_ec));
          else
            *ec = local_ec;
          return false;
        }
  ...
  bool create_directory(const path& p, error_code* ec)
  {
    ...
    if (ec == 0)
      BOOST_FILESYSTEM_THROW(filesystem_error("hexerboost::filesystem::create_directory",
        p, error_code(errval, system_category())));
    else
...
#   if defined(BOOST_WINDOWS_API) && _WIN32_WINNT < 0x0600  // SDK earlier than Vista and Server 2008
    ...
    error(true, error_code(BOOST_ERROR_NOT_SUPPORTED, system_category()), to, from, ec,
      "hexerboost::filesystem::create_directory_symlink");
#   else
...
#     endif
    ...
    error(!BOOST_CREATE_SYMBOLIC_LINK(from.c_str(), to.c_str(), SYMBOLIC_LINK_FLAG_DIRECTORY),
      to, from, ec, "hexerboost::filesystem::create_directory_symlink");
#   endif
  }
...
#   if defined(BOOST_WINDOWS_API) && _WIN32_WINNT < 0x0500  // SDK earlier than Win 2K
    ...
    error(true, error_code(BOOST_ERROR_NOT_SUPPORTED, system_category()), to, from, ec,
      "hexerboost::filesystem::create_hard_link");
#   else
...
#     if defined(BOOST_WINDOWS_API) && _WIN32_WINNT >= 0x0600
        ...
        if (error(!create_symbolic_link_api,
            error_code(BOOST_ERROR_NOT_SUPPORTED, system_category()),
            to, from, ec,
            "hexerboost::filesystem::create_symlink"))
          return;
#     endif

    error(!BOOST_CREATE_SYMBOLIC_LINK(from.c_str(), to.c_str(), 0),
      to, from, ec, "hexerboost::filesystem::create_symlink");
#   endif
  }
...
#         if defined(__MSL__) && (defined(macintosh) || defined(__APPLE__) || defined(__APPLE_CC__))
          && errno != 0
#         endif
          , ec, "hexerboost::filesystem::current_path"))
        {
          break;
        }
      }
...
#   else
    ...
    hexerboost::scoped_array<path::value_type> buf(new path::value_type[sz]);
    error(::GetCurrentDirectoryW(sz, buf.get())== 0, ec,
      "hexerboost::filesystem::current_path");
    return path(buf.get());
#   endif
  }
  ...
  void current_path(const path& p, system::error_code* ec)
  {
    error(!BOOST_SET_CURRENT_DIRECTORY(p.c_str()),
      p, ec, "hexerboost::filesystem::current_path");
  }
...
#   ifdef BOOST_POSIX_API
    ...
    if (e1 != 0 || e2 != 0)
    {
      ...
      error (e1 != 0 && e2 != 0, p1, p2, ec, "hexerboost::filesystem::equivalent");
      return false;
    }
...
#   else  // Windows
      ...
      || h2.handle == INVALID_HANDLE_VALUE)
    {
      ...
      error(h1.handle == INVALID_HANDLE_VALUE
        && h2.handle == INVALID_HANDLE_VALUE, p1, p2, ec,
          "hexerboost::filesystem::equivalent");
      return false;
    }
    ...
    if (error(!::GetFileInformationByHandle(h1.handle, &info1),
      p1, p2, ec, "hexerboost::filesystem::equivalent"))
        return  false;
...
#   endif
  ...
  BOOST_FILESYSTEM_DECL
  hexerboost::uintmax_t file_size(const path& p, error_code* ec)
  {
#   ifdef BOOST_POSIX_API

    struct stat path_stat;
    if (error(::stat(p.c_str(), &path_stat)!= 0,
        p, ec, "hexerboost::filesystem::file_size"))
      return static_cast<hexerboost::uintmax_t>(-1);
   if (error(!S_ISREG(path_stat.st_mode),
      error_code(EPERM, system_category()),
        p, ec, "hexerboost::filesystem::file_size"))
      return static_cast<hexerboost::uintmax_t>(-1);

    return static_cast<hexerboost::uintmax_t>(path_stat.st_size);
...
#   else  // Windows
    ...
    return (static_cast<hexerboost::uintmax_t>(fad.nFileSizeHigh)
              << (sizeof(fad.nFileSizeLow)*8)) + fad.nFileSizeLow;
#   endif
  }

  BOOST_FILESYSTEM_DECL
  hexerboost::uintmax_t hard_link_count(const path& p, system::error_code* ec)
  {
#   ifdef BOOST_POSIX_API

    struct stat path_stat;
    return error(::stat(p.c_str(), &path_stat)!= 0,
                  p, ec, "hexerboost::filesystem::hard_link_count")
           ? 0
           : static_cast<hexerboost::uintmax_t>(path_stat.st_nlink);
...
#   ifdef BOOST_POSIX_API
    ...
    struct stat path_stat;
    if (error(::stat(p.c_str(), &path_stat)!= 0,
        p, ec, "hexerboost::filesystem::is_empty"))
      return false;        
...
#   ifdef BOOST_POSIX_API
    ...
    struct stat path_stat;
    if (error(::stat(p.c_str(), &path_stat)!= 0,
      p, ec, "hexerboost::filesystem::last_write_time"))
        return std::time_t(-1);
...
# ifdef BOOST_POSIX_API
    ...
    if (local_ec)
    {
      if (ec == 0)
      BOOST_FILESYSTEM_THROW(filesystem_error(
          "hexerboost::filesystem::permissions", p, local_ec));
      else
        *ec = local_ec;
      return;
    }
...
#   endif
    {
      if (ec == 0)
      BOOST_FILESYSTEM_THROW(filesystem_error(
          "hexerboost::filesystem::permissions", p,
...
# else  // Windows
    ...
    if (error(attr == 0, p, ec, "hexerboost::filesystem::permissions"))
      return;
    ...
    error(::SetFileAttributesW(p.c_str(), attr) == 0,
      p, ec, "hexerboost::filesystem::permissions");
# endif
  }
...
#   ifdef BOOST_POSIX_API
    ...
    {
      hexerboost::scoped_array<char> buf(new char[path_max]);
      ssize_t result;
      if ((result=::readlink(p.c_str(), buf.get(), path_max))== -1)
      {
        if (ec == 0)
          BOOST_FILESYSTEM_THROW(filesystem_error("hexerboost::filesystem::read_symlink",
...
#   elif _WIN32_WINNT < 0x0600  // SDK earlier than Vista and Server 2008
    error(true, error_code(BOOST_ERROR_NOT_SUPPORTED, system_category()), p, ec,
          "hexerboost::filesystem::read_symlink");
#   else  // Vista and Server 2008 SDK, or later
    ...
    if (error(h.handle == INVALID_HANDLE_VALUE, p, ec, "hexerboost::filesystem::read_symlink"))
      return symlink_path;
...
#     endif
  ...
  BOOST_FILESYSTEM_DECL
  hexerboost::uintmax_t remove_all(const path& p, error_code* ec)
  {
    error_code tmp_ec;
    file_type type = query_file_type(p, &tmp_ec);
    if (error(type == status_error, tmp_ec, p, ec,
      "hexerboost::filesystem::remove_all"))
      return 0;
  ...
  void rename(const path& old_p, const path& new_p, error_code* ec)
  {
    error(!BOOST_MOVE_FILE(old_p.c_str(), new_p.c_str()), old_p, new_p, ec,
      "hexerboost::filesystem::rename");
  }

  BOOST_FILESYSTEM_DECL
  void resize_file(const path& p, uintmax_t size, system::error_code* ec)
  {
    error(!BOOST_RESIZE_FILE(p.c_str(), size), p, ec, "hexerboost::filesystem::resize_file");
  }
...
#   ifdef BOOST_POSIX_API
    ...
    if (!error(::BOOST_STATVFS(p.c_str(), &vfs)!= 0,
      p, ec, "hexerboost::filesystem::space"))
    {
      info.capacity 
        = static_cast<hexerboost::uintmax_t>(vfs.f_blocks)* BOOST_STATVFS_F_FRSIZE;
      info.free 
        = static_cast<hexerboost::uintmax_t>(vfs.f_bfree)* BOOST_STATVFS_F_FRSIZE;
      info.available
        = static_cast<hexerboost::uintmax_t>(vfs.f_bavail)* BOOST_STATVFS_F_FRSIZE;
    }
...
#   else
    ...
    {
      info.capacity
        = (static_cast<hexerboost::uintmax_t>(total.HighPart)<< 32)
          + total.LowPart;
      info.free
        = (static_cast<hexerboost::uintmax_t>(free.HighPart)<< 32)
          + free.LowPart;
      info.available
        = (static_cast<hexerboost::uintmax_t>(avail.HighPart)<< 32)
          + avail.LowPart;
    }
...
#   ifdef BOOST_POSIX_API
      ...
      {
        errno = ENOTDIR;
        error(true, p, ec, "hexerboost::filesystem::temp_directory_path");
        return p;
      }
...
#   else  // Windows
      ...
      if (buf.empty() || GetTempPathW(buf.size(), &buf[0])==0)
      {
        ...
        error(true, ec, "hexerboost::filesystem::temp_directory_path");
        return path();
      }
...
#   else
    ...
    if (error(len == 0, p, ec, "hexerboost::filesystem::system_complete"))
      return path();

    if (len < buf_size)// len does not include null termination character
      return path(&buf[0]);

    hexerboost::scoped_array<wchar_t> big_buf(new wchar_t[len]);

    return error(get_full_path_name(p, len , big_buf.get(), &pfn)== 0,
      p, ec, "hexerboost::filesystem::system_complete")
      ? path()
      : path(big_buf.get());
#   endif
  }
...
#   endif
    ...
    const path& p, system::error_code* ec)    
  {
    if (error(p.empty(), not_found_error_code, p, ec,
              "hexerboost::filesystem::directory_iterator::construct"))
      return;
...
#     endif
    ...
    if (result)
    {
      it.m_imp.reset();
      error(true, result, p,
        ec, "hexerboost::filesystem::directory_iterator::construct");
      return;
    }
      ...
      if (temp_ec)  // happens if filesystem is corrupt, such as on a damaged optical disc
      {
        ...
        if (ec == 0)
          BOOST_FILESYSTEM_THROW(
            filesystem_error("hexerboost::filesystem::directory_iterator::operator++",
              error_path,
              error_code(BOOST_ERRNO, system_category())));
        ec->assign(BOOST_ERRNO, system_category());
        return;
