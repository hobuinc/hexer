...
namespace user
{
    ...
    const internal_string_type & src )
  {
    ...
    hexerboost::scoped_array<char> work( new char[ work_size ] );
    std::mbstate_t state;
    ...
    if ( cvt->out( 
      ...
      hexerboost::throw_exception<fs::basic_filesystem_error<mbpath> >(
        fs::basic_filesystem_error<mbpath>(
          "user::mbpath::to_external conversion error",
          ph, hexerboost::system::error_code( EINVAL, hexerboost::system::errno_ecat ) ) );
    *to_next = '\0';
  ...
  mbpath_traits::to_internal( const external_string_type & src )
  {
      ...
      hexerboost::scoped_array<wchar_t> work( new wchar_t[ work_size ] );
      std::mbstate_t state;
      ...
      if ( cvt->in( 
        ...
        hexerboost::throw_exception<fs::basic_filesystem_error<mbpath> >(
          fs::basic_filesystem_error<mbpath>(
            ...
            hexerboost::system::error_code( EINVAL, hexerboost::system::errno_ecat ) ) );
      *to_next = L'\0';
