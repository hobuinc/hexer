...
//[case_insensitive_functions
        ...
        : std::binary_function<std::string, std::string, bool>
    {
        ...
        {
            return hexerboost::algorithm::iequals(x, y, std::locale());
        }
    };
        ...
        : std::unary_function<std::string, std::size_t>
    {
        std::size_t operator()(std::string const& x) const
        {
            ...
            for(std::string::const_iterator it = x.begin();
                it != x.end(); ++it)
            {
                hexerboost::hash_combine(seed, std::toupper(*it, locale));
            }
...
//[case_sensitive_dictionary_fnv
    hexerboost::unordered_map<std::string, int, hash::fnv_1>
        dictionary;
//]
...
//[case_insensitive_dictionary
    hexerboost::unordered_map<std::string, int, ihash, iequal_to>
        idictionary;
//]
