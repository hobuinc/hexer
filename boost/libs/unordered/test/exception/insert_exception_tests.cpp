...
struct insert_test_rehash1 : public insert_test_base<T>
{
    ...
    T init() const {
        ...
        x.insert(this->values.begin(),
                hexerboost::next(this->values.begin(), initial_elements));
    ...
    void run(T& x, strong_type& strong) const {
        ...
        for(BOOST_DEDUCED_TYPENAME test::random_values<T>::const_iterator
            it = hexerboost::next(this->values.begin(), x.size()),
                end = this->values.end();
...
struct insert_test_rehash3 : public insert_test_base<T>
{
    ...
    void run(T& x) const {
        ...
        x.insert(hexerboost::next(this->values.begin(), x.size()),
                hexerboost::next(this->values.begin(), x.size() + 20));
