...
void insert_tests2(X*, test::random_generator generator)
{
...

    {
            ...
            it = v.begin(); it != v.end(); ++it)
        {
            ...
            x.insert(it, hexerboost::next(it));
            tracker.insert(*it);
            tracker.compare_key(x, *it);
...
void move_emplace_tests(X*, test::random_generator generator)
{
            ...
            it != v.end(); ++it)
    {
        ...
        x.emplace(hexerboost::move(value));
        tracker.insert(*it);
        tracker.compare_key(x, *it);
...
void map_insert_range_test2(X*, test::random_generator generator)
{
    ...
    > list;
    test::random_values<
        hexerboost::unordered_map<BOOST_DEDUCED_TYPENAME X::key_type, test::implicitly_convertible>
    > v(1000, generator);
    list l(v.begin(), v.end());
...
hexerboost::unordered_set<test::movable,
    test::hash, test::equal_to,
    std::allocator<test::movable> >* test_set_std_alloc;
hexerboost::unordered_multimap<test::object, test::object,
    test::hash, test::equal_to,
    std::allocator<test::object> >* test_multimap_std_alloc;

hexerboost::unordered_set<test::object,
    test::hash, test::equal_to,
    test::allocator1<test::object> >* test_set;
hexerboost::unordered_multiset<test::movable,
    test::hash, test::equal_to,
    test::allocator2<test::movable> >* test_multiset;
hexerboost::unordered_map<test::movable, test::movable,
    test::hash, test::equal_to,
...
{
    hexerboost::unordered_set<int> set;
    set.insert({1,2,3,1});
...
{
    hexerboost::unordered_multiset<std::string> multiset;
    //multiset.insert({});
...
{
    hexerboost::unordered_map<std::string, std::string> map;
    //map.insert({});
...
{
    hexerboost::unordered_multimap<std::string, std::string> multimap;
    //multimap.insert({});
...
struct overloaded_constructor
{
    ...
    {
        std::size_t hash = 0;
        hexerboost::hash_combine(hash, x.x1);
        hexerboost::hash_combine(hash, x.x2);
        hexerboost::hash_combine(hash, x.x3);
        hexerboost::hash_combine(hash, x.x4);
        return hash;
    }
};
...
{
    hexerboost::unordered_map<int, overloaded_constructor> x;
...
{
    hexerboost::unordered_set<overloaded_constructor> x;
...
struct convertible_to_piecewise {
    operator hexerboost::unordered::piecewise_construct_t() const {
        return hexerboost::unordered::piecewise_construct;
    }
};

UNORDERED_AUTO_TEST(map_emplace_test2)
{
    hexerboost::unordered_map<overloaded_constructor, overloaded_constructor> x;

    x.emplace(hexerboost::unordered::piecewise_construct, hexerboost::make_tuple(), hexerboost::make_tuple());
    BOOST_TEST(x.find(overloaded_constructor()) != x.end() &&
        x.find(overloaded_constructor())->second == overloaded_constructor());

    x.emplace(convertible_to_piecewise(), hexerboost::make_tuple(1), hexerboost::make_tuple());
    BOOST_TEST(x.find(overloaded_constructor(1)) != x.end() &&
        x.find(overloaded_constructor(1))->second == overloaded_constructor());

    x.emplace(piecewise_rvalue(), hexerboost::make_tuple(2,3), hexerboost::make_tuple(4,5,6));
    BOOST_TEST(x.find(overloaded_constructor(2,3)) != x.end() &&
        x.find(overloaded_constructor(2,3))->second == overloaded_constructor(4,5,6));

    derived_from_piecewise_construct_t d;
    x.emplace(d, hexerboost::make_tuple(9,3,1), hexerboost::make_tuple(10));
...
{
    hexerboost::unordered_set<std::pair<overloaded_constructor, overloaded_constructor> > x;
    ...
    x.clear();
    x.emplace(hexerboost::unordered::piecewise_construct, hexerboost::make_tuple(1), hexerboost::make_tuple(2,3));
