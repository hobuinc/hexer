    ...
    std::cerr<<"erase(random position).\n";
    {
        ...
        while(size > 0 && !x.empty())
        {
            ...
            if(index == 0) {
                ...
                prev = hexerboost::next(x.begin(), index - 1);
                pos = hexerboost::next(prev);
            }
            next = hexerboost::next(pos);
            BOOST_DEDUCED_TYPENAME Container::key_type
...
hexerboost::unordered_multiset<test::object,
    test::hash, test::equal_to,
    test::allocator2<test::object> >* test_multiset;
hexerboost::unordered_map<test::object, test::object,
    test::hash, test::equal_to,
