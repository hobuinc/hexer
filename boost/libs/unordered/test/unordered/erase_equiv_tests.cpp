...
typedef hexerboost::unordered_multimap<int, int,
...
UNORDERED_AUTO_TEST(two_equivalent_item_tests)
{
    ...
    {
        collide_map x(init.begin(), init.end());
        int value = hexerboost::next(x.begin())->second;
        x.erase(x.begin(), hexerboost::next(x.begin()));
        BOOST_TEST(x.count(1) == 1 && x.size() == 1 &&
...

    {
        ...
        x.erase(hexerboost::next(x.begin()), x.end());
        BOOST_TEST(x.count(1) == 1 && x.size() == 1 &&
                x.begin()->first == 1 && x.begin()->second == value);
        test::check_equivalent_keys(x);
    }
...
bool general_erase_range_test(Container& x, std::size_t start, std::size_t end)
{
    ...
    l.erase(hexerboost::next(l.begin(), start), hexerboost::next(l.begin(), end));
    x.erase(hexerboost::next(x.begin(), start), hexerboost::next(x.begin(), end));
