...
UNORDERED_AUTO_TEST(set_tests)
{
    ...
    typedef hexerboost::unordered_set<int> set;
    typedef hexerboost::unordered_multiset<int> multiset;
...
UNORDERED_AUTO_TEST(map_tests)
{
    ...
    for(int i = 0; i < 5; ++i)
        test_equal_insertion<hexerboost::unordered_map<int, int> >(
            v[i].begin(), v[i].end());

    for(int i2 = 0; i2 < 5; ++i2)
        test_equal_insertion<hexerboost::unordered_multimap<int, int> >(
            v[i2].begin(), v[i2].end());
}
