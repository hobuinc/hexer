...
namespace hexerboost {} namespace boost = hexerboost; namespace hexerboost { namespace program_options { namespace detail {
    ...
    common_config_file_iterator::add_option(const char* name)
    {
        ...
        if (*s.rbegin() == '*') {
            ...
            if (bad_prefixes)
                hexerboost::throw_exception(error("options '" + string(name) + "' and '" +
            ...
            if (!s.empty()) {
                ...
                if (*s.begin() == '[' && *s.rbegin() == ']') {
                    ...
                    if (!registered && !m_allow_unregistered)
                        hexerboost::throw_exception(unknown_option(name));
                    ...
                    hexerboost::throw_exception(invalid_config_file_syntax(s, invalid_syntax::unrecognized_line));
                }
            }
        }
        if (!found)
...
#if 0
using hexerboost::program_options::config_file;
